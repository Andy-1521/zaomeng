...
- SAM 后处理：默认 Clown 会对 RunningHub 返回的 mask 做二值化、闭运算、连通块去噪、重复 mask 去重和区域数量限制，再合成纯色 PNG，减少碎噪点。
- 原图量化兜底：如果 RunningHub/SAM 返回空 mask，后端会用彩绘结果图自身低分辨率平滑量化生成对齐的纯色分区 PNG，写入 `clownWorkflowMode=source-quantize-fallback`。该兜底不会重绘，能保持坐标对齐，但语义精度不如真实 mask。
- GPT 实验路线：后端保留 `variant: "gpt"` 的实验分支，但因为实测会重绘导致对不齐，用户界面不展示该按钮，不作为默认方案。
