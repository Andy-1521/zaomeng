...
- 未配置 RunningHub Clown 工作流时，接口返回“Clown 分割工作流未配置”，不扣积分。
...
工作流输入是彩绘提取结果图 URL，输出应是一张同画幅的纯色分区 PNG。第一版优先读取 RunningHub 直接输出的 PNG；如果后续工作流改为输出多张 mask，需要再补后端 mask 合成逻辑。未配置这些变量时，前端按钮可点击但接口会明确提示未配置且不扣积分。
