...
const diagnosticOptions: DiagnosticOption[] = [
  {
    ...
    description: "成功或处理中但没有有效结果图",
  },
  {
    label: "上游失败",
    value: "upstream-error",
    description: "上游服务明确返回失败",
  },
  {
    label: "超时异常",
    value: "timeout-error",
    description: "请求超时或连接中断",
  },
  {
    label: "其他异常",
    value: "other-failure",
    description: "未归类的失败订单",
  },
...
function getNormalizedToolLabel(
  toolPage: string,
  description: string,
  orderNumber: string,
): string {
...
function getSmartEditAgentInfo(
  record: GenerationRecord | null,
): SmartEditAgentInfo | null {
  if (!record) return null;

  const toolLabel = getNormalizedToolLabel(
    record.toolPage,
    record.description || "",
    record.orderNumber || "",
  );
  ...
  return {
    ...
    summary:
      ...
      getStringParam(params, "promptSummary"),
    ...
    requestedAspectRatio:
      getStringParam(params, "requestedAspectRatio") ||
      getStringParam(params, "requestedSize"),
    resolvedAspectRatio:
      getStringParam(params, "resolvedAspectRatio") ||
      getStringParam(params, "resolvedSize"),
    mode: getStringParam(params, "mode"),
    regionCount: getNumberParam(params, "regionCount"),
  };
}

function getRequestImageUrls(
  params: RequestParamsValue,
  fallbackUploadedImage?: string,
): string[] {
  const urls: string[] = [];
...
function getResultImageUrls(data: ResultDataValue): string[] {
  ...
  if (typeof data === "object" && data !== null) {
    if (Array.isArray(data.result_image_url)) {
      return data.result_image_url.filter(
...
function getRecordDiagnostic(
  record: Pick<
    GenerationRecord,
    "status" | "resultData" | "requestParams" | "uploadedImage"
  >,
): RecordDiagnostic {
  const resultImageCount = getResultImageUrls(record.resultData).length;
  const requestImageCount = getRequestImageUrls(
  ...
  const hasUpstreamError = /upstream_error|Upstream request failed/i.test(
    errorMessage,
  );
...
function getDiagnosticRecommendations(
  record: Pick<
    GenerationRecord,
    "toolPage" | "status" | "requestParams" | "uploadedImage"
  >,
    ...
    return [
      {
        title: "复核上游图像编辑链路",
        detail:
          "优先检查 OpenAI 兼容 images/edits 接口是否在同时间段返回 502、upstream_error 或网关错误。",
      },
      {
        title: "重放同类请求",
    ...
    return [
      {
        title: "检查超时点位",
        detail:
          "先确认是应用侧等待超时、网络中断，还是上游处理时间过长导致的连接超时。",
      },
      {
        title: "核对输入尺寸与复杂度",
      ...
      },
      {
        title: "复查重试策略",
        detail:
          "确认当前工具是否需要增加重试、缩短单次处理链路，或在超时后补偿回写失败状态。",
      },
    ];
  }
    ...
    return [
      {
        title: "检查结果入库链路",
        detail:
          "重点核对任务执行成功后，结果 URL 是否正确写回 transaction.resultData。",
      },
      {
        title: "检查存储上传结果",
        detail:
          "确认结果图是否已经生成但上传存储失败，或返回结构变化导致前端没有识别出 imageUrl/result_image_url。",
      },
      {
        title: "核对任务真实状态",
    ...
    return [
      {
        title: "先看原始请求与结果数据",
        detail:
          "这类订单没有命中已知的上游失败或超时特征，优先查看下方原始请求参数和结果数据。",
      },
      {
        title: "按工具维度对比",
      ...
      },
      {
        title: "必要时做单单复现",
        detail:
          "如果错误信息不完整，建议在测试环境或日志中重放该订单，补齐更明确的异常来源。",
      },
    ];
  }
  ...
  return [
    {
      title: "结果已正常产出",
      detail:
        "当前订单没有明显异常。如果用户仍反馈问题，优先人工核对结果图质量与业务预期是否一致。",
    },
    {
      title: "必要时检查输入输出一致性",
...
function AdminFilterDropdown({
  ...
  return (
    <div
      ...
      data-role="admin-filter-dropdown"
    >
      <button
        type="button"
      ...
      >
        <span className={`truncate ${value ? "text-white" : "text-white/70"}`}>
      ...
      {isOpen ? (
        <div
          className={`absolute ${positionClassName} top-full z-30 mt-2 max-w-[calc(100vw-2rem)] ${menuWidthClassName} overflow-hidden rounded-[1rem] border border-white/12 bg-[#0d0d12]/98 p-1 shadow-[0_18px_40px_rgba(0,0,0,0.4)] backdrop-blur-xl`}
        >
          {options.map((option) => {
            ...
            return (
              ...
              >
                <span className="flex min-w-0 flex-1 flex-col">
                  ...
                  {option.description ? (
                    <span
                      className={`mt-0.5 text-[11px] leading-4 ${selected ? "text-purple-100/70" : "text-white/42"}`}
                    >
                      {option.description}
                    </span>
                  ) : null}
                ...
                {selected ? (
                  <span className="pt-0.5 text-[10px] text-purple-100">✓</span>
                ) : null}
              </button>
            );
          })}
...
export default function AdminGenerationsPage() {
  ...
  const [generatedRechargeCode, setGeneratedRechargeCode] =
    useState<RechargeCodeRecord | null>(null);
  const [isCreatingRechargeCode, setIsCreatingRechargeCode] = useState(false);
  const [rechargeCodeStatusFilter, setRechargeCodeStatusFilter] =
    useState<RechargeCodeStatusFilter>("all");
  const [rechargeCodeKeyword, setRechargeCodeKeyword] = useState("");
  const [voidingRechargeCodeId, setVoidingRechargeCodeId] = useState<
    string | null
  >(null);
  ...
  const [openDropdownId, setOpenDropdownId] = useState<AdminDropdownId | null>(
    null,
  );
  ...
  const [marketStats, setMarketStats] =
    useState<MarketStatsState>(EMPTY_MARKET_STATS);
  ...
  const successRate =
    totalStats.total > 0
      ...
      : 0;
  const totalPointsConsumed = records.reduce(
    (sum, r) => sum + (r.actualPoints > 0 ? r.actualPoints : 0),
    0,
  );
  const processingCount =
    totalStats.processingCount ||
    records.filter(
      (record) => record.status === "处理中" || record.status === "pending",
    ).length;

  const derivedToolStats = records.reduce<Record<string, number>>(
    (acc, record) => {
      const toolLabel = getNormalizedToolLabel(
        record.toolPage,
        record.description || "",
  ...
  const activeToolSummary = adminToolOptions
    .map((option) => ({
      ...option,
      count: derivedToolStats[option.label] || 0,
    }))
  ...
  const filteredRechargeCodes = useMemo(() => {
    ...
    return rechargeCodes.filter((item) => {
      if (
        rechargeCodeStatusFilter !== "all" &&
        item.status !== rechargeCodeStatusFilter
      ) {
        return false;
      }
  ...
  const loadRecords = useCallback(
    async (pageNum: number = 0, filters?: Partial<RecordFilters>) => {
      ...
      try {
        ...
        const keyword =
          filters?.keyword !== undefined ? filters.keyword : searchKeyword;
        const toolPage =
          filters?.toolPage !== undefined ? filters.toolPage : filterToolPage;
        const status =
          filters?.status !== undefined ? filters.status : filterStatus;
        const diagnostic =
          filters?.diagnostic !== undefined
            ? filters.diagnostic
            : filterDiagnostic;
        const startDate =
          filters?.startDate !== undefined
            ...
            : filterStartDate;
        const endDate =
          filters?.endDate !== undefined ? filters.endDate : filterEndDate;
        ...
        if (data.success) {
          if (pageNum === 0) {
            ...
            const newStats = {
              ...
              failureBreakdown: {
                upstreamErrorCount:
                  data.data.stats?.failureBreakdown?.upstreamErrorCount || 0,
                timeoutErrorCount:
                  data.data.stats?.failureBreakdown?.timeoutErrorCount || 0,
                missingResultCount:
                  data.data.stats?.failureBreakdown?.missingResultCount || 0,
                otherFailureCount:
                  data.data.stats?.failureBreakdown?.otherFailureCount || 0,
              },
            };
            ...
            setRecords((prev) => {
              ...
              const newRecords = data.data.records.filter(
  ...
  const getCurrentFilters = useCallback(
    (overrides?: Partial<RecordFilters>): RecordFilters => {
      return {
  ...
  const triggerRecordSearch = useCallback(
  ...
  const loadRechargeCodes = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setError(
          toUserFacingErrorMessage(data.message, "加载兑换码失败，请稍后重试"),
        );
      }
    } catch {
  ...
  const createRechargeCode = async () => {
    if (
      !Number.isInteger(rechargeCodeAmount) ||
      rechargeCodeAmount < 1 ||
      rechargeCodeAmount > 5000
    ) {
    ...
    try {
      ...
      if (!response.ok || !data.success || !data.data) {
        showToast(
          toUserFacingErrorMessage(data.message, "生成兑换码失败，请稍后重试"),
          "error",
        );
        return;
      }
  ...
  const getCustomerSummary = useCallback((record: GenerationRecord) => {
    ...
    const requestImages = getRequestImageUrls(
      record.requestParams,
      record.uploadedImage,
    );
    ...
    return [
      ...
      diagnostic.errorMessage && diagnostic.errorMessage !== "未知错误"
        ? `原因: ${diagnostic.errorMessage}`
        : `摘要: ${diagnostic.summary}`,
    ].join("\n");
  }, []);
  ...
  const copyVisibleIssueSummaries = useCallback(async () => {
    ...
    await copyText(
      issueRecords
        .map((record) => getCustomerSummary(record))
        .join("\n\n---\n\n"),
      `已复制 ${issueRecords.length} 条异常摘要`,
    );
  }, [copyText, getCustomerSummary, records]);

  const downloadRecordResults = useCallback(
    async (record: GenerationRecord) => {
      ...
      try {
        ...
        showToast(
          toUserFacingErrorFromUnknown(error, "结果图下载失败，请重试"),
          "error",
        );
      }
    },
  ...
  const voidRechargeCode = useCallback(
    async (item: RechargeCodeRecord) => {
      ...
      try {
        ...
        setRechargeCodes((current) =>
          current.map((code) => (code.id === item.id ? data.data! : code)),
        );
        ...
        showToast(
          toUserFacingErrorFromUnknown(error, "作废兑换码失败，请稍后重试"),
          "error",
        );
      } finally {
  ...
  const refreshSession = useCallback(async () => {
    try {
      ...
      if (!userStr) {
        ...
        router.push("/login?next=/admin/generations");
        return;
      }
      ...
      if (data.success) {
        ...
        showToast(
          toUserFacingErrorMessage(data.message, "管理员权限校验失败"),
          "error",
        );
        if (!user?.isAdmin) {
          router.push("/home");
    ...
    try {
      ...
      if (data.success) {
        setSelectedUser((current) =>
          current?.id === userId
            ? { ...current, isAdmin: !currentIsAdmin }
            : current,
        );
        void loadUsers();
        ...
        showToast(
          toUserFacingErrorMessage(data.message, "操作失败，请稍后重试"),
          "error",
        );
      }
    } catch {
  ...
  const handleQuickFilter = (
    ...
    const newDiagnostic =
      nextDiagnostic !== undefined
        ? nextDiagnostic === filterDiagnostic
          ? ""
          : nextDiagnostic
        : filterDiagnostic;
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <div className="inline-flex items-center gap-1.5">
        <span
          className={`w-2 h-2 rounded-full ${config.dotClass} ${status === "处理中" ? "animate-pulse" : ""}`}
        />
  ...
  const renderToolBadge = (record: GenerationRecord) => {
    ...
    return (
      <span
        className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-medium ${getToolBadgeClass(toolLabel)}`}
      >
        {toolLabel}
      </span>
    );
  ...
  const openEditModal = (
    ...
    if (type === "points") {
      setEditValue(
        typeof currentValue === "number" ? currentValue.toString() : "",
      );
      return;
    }
    ...
    try {
      ...
      const adminUserId =
        ...
        (localStorage.getItem("user")
          ...
          : "");
      const response = await fetch(
        `/api/admin/update-user?userId=${adminUserId}`,
        {
          method: "POST",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
      ...
      if (data.success) {
        ...
        showToast(
          toUserFacingErrorMessage(data.message, "更新失败，请稍后重试"),
          "error",
        );
      }
    } catch {
  ...
  const detailResultImages = detailRecord
    ? getResultImageUrls(detailRecord.resultData)
    : [];
  const detailRequestImages = detailRecord
    ? getRequestImageUrls(
        detailRecord.requestParams,
        detailRecord.uploadedImage,
      )
    : [];
  const detailDiagnostic = detailRecord
    ? getRecordDiagnostic(detailRecord)
    : null;
  const detailSmartEditAgent = getSmartEditAgentInfo(detailRecord);
  const detailRecommendations =
    detailRecord && detailDiagnostic
      ? getDiagnosticRecommendations(detailRecord, detailDiagnostic)
      : [];
  const detailCustomerSummary = detailRecord
    ? getCustomerSummary(detailRecord)
    : "";
    ...
    return (
      <div className="min-h-screen bg-black text-white flex items-center justify-center relative overflow-hidden">
        <div className="absolute inset-0">
          ...
          <div
            className="absolute bottom-1/4 right-1/4 w-[700px] h-[700px] bg-blue-600/12 rounded-full blur-[120px] animate-pulse"
        ...
        <div className="relative z-10 flex flex-col flex-1 min-h-0 px-5 py-3">
          <div className="max-w-7xl mx-auto w-full flex flex-col flex-1 min-h-0">
                ...
                <section className="rounded-xl border border-white/10 bg-white/[0.04] p-4 flex-shrink-0">
                  ...
                  <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
                    <div className="rounded-xl border border-purple-300/16 bg-purple-500/[0.08] p-4">
                      ...
                      <div className="mt-1 text-3xl font-semibold tabular-nums text-purple-100">
                        {marketStats.totalItems}
                      </div>
                      <div className="mt-1 text-xs text-purple-100/42">
                        PSD {marketStats.psdItems} 个
                      </div>
                    </div>
                    <div className="rounded-xl border border-amber-300/16 bg-amber-500/[0.08] p-4">
                      <div className="text-xs text-amber-100/60">待审核</div>
                      <div className="mt-1 text-3xl font-semibold tabular-nums text-amber-200">
                        {marketStats.pendingItems}
                      </div>
                      <div className="mt-1 text-xs text-amber-100/42">
                        已上架 {marketStats.approvedItems} 个
                      </div>
                    </div>
                    <div className="rounded-xl border border-emerald-300/16 bg-emerald-500/[0.08] p-4">
                      ...
                      <div className="mt-1 text-3xl font-semibold tabular-nums text-emerald-200">
                        {marketStats.totalPurchases}
                      </div>
                      <div className="mt-1 text-xs text-emerald-100/42">
                        今日 {marketStats.todayPurchases} 单
                      </div>
                    </div>
                    <div className="rounded-xl border border-cyan-300/16 bg-cyan-500/[0.08] p-4">
                      <div className="text-xs text-cyan-100/60">平台收益</div>
                      <div className="mt-1 text-3xl font-semibold tabular-nums text-cyan-200">
                        {marketStats.totalPlatformFeePoints}
                      </div>
                      <div className="mt-1 text-xs text-cyan-100/42">
                        成交 {marketStats.totalSalesPoints} / 卖家{" "}
                        {marketStats.totalSellerPoints}
                      </div>
                    </div>
                  </div>
                </section>
                ...
                <section className="grid gap-3 md:grid-cols-3">
                  <div className="rounded-xl border border-white/10 bg-white/[0.04] p-4">
                    ...
                    <div className="mt-4 space-y-3 text-sm">
                      <div className="flex items-center justify-between text-white/58">
                        <span>已上架</span>
                        <span className="tabular-nums text-white/88">
                          {marketStats.approvedItems}
                        </span>
                      </div>
                      <div className="flex items-center justify-between text-white/58">
                        <span>待审核</span>
                      ...
                      <div className="flex items-center justify-between text-white/58">
                        ...
                        <span className="tabular-nums text-red-200">
                          {marketStats.rejectedItems}
                        </span>
                      </div>
                      <div className="flex items-center justify-between text-white/58">
                        <span>已下架</span>
                        <span className="tabular-nums text-white/70">
                          {marketStats.delistedItems}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="rounded-xl border border-white/10 bg-white/[0.04] p-4">
                    ...
                    <div className="mt-4 space-y-3 text-sm text-white/52">
                      <p>
                        待审核大于 0
                        时，优先进入图市待审核处理，避免用户上架素材长时间不展示。
                      </p>
                      <p>
                        PSD 数量可以反映高价值素材占比，后续可以按 PSD
                        素材做单独筛选和推荐。
                      </p>
                    </div>
                  </div>
                ...
                <div className="flex flex-shrink-0 flex-col gap-3 rounded-xl border border-white/10 bg-white/[0.04] p-3 sm:flex-row sm:items-center">
                  <div className="min-w-0 flex-1">
                    <input
                      ...
                      onKeyDown={(e) =>
                        e.key === "Enter" && void loadUsers(userSearchKeyword)
                      }
                  ...
                  <div className="text-xs text-white/42 sm:ml-auto">
                    当前 {users.length} 个用户
                  </div>
                </div>
                      ...
                      <table className="w-full min-w-[760px]">
                        <thead className="sticky top-0 z-[1]">
                          <tr className="border-b border-white/10 bg-neutral-900/95 backdrop-blur">
                            <th className="text-left px-5 py-3 text-xs font-semibold text-white/60 uppercase tracking-wider">
                              用户
                            </th>
                            <th className="text-left px-5 py-3 text-xs font-semibold text-white/60 uppercase tracking-wider">
                              邮箱
                              ...
                              >
                                <td className="px-5 py-3">
                                  <div className="flex items-center gap-3">
                                    <div
                                      className="relative h-8 w-8 cursor-pointer overflow-hidden rounded-full group"
                                      onClick={(event) => {
                                ...
                                <td className="px-5 py-3 text-sm text-white/60">
                                  <button
                                    ...
                                    onClick={(event) => {
                                      event.stopPropagation();
                                      void copyText(user.email, "邮箱已复制");
                                    }}
                                ...
                                <td className="px-5 py-3">
                                  {user.isAdmin ? (
                                    ...
                                    <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-white/20 text-white/60">
                                      普通用户
                                    </span>
                                  )}
                                </td>
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="space-y-2 text-sm">
                            <button
                              type="button"
                              onClick={() =>
                                void copyText(selectedUser.id, "UID 已复制")
                              }
                              className="flex w-full items-center justify-between gap-3 rounded-xl border border-white/10 bg-black/20 px-3 py-2 text-left text-white/68 hover:bg-white/8 hover:text-white"
                            >
                              <span>UID</span>
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                void copyText(selectedUser.email, "邮箱已复制")
                              }
                              className="flex w-full items-center justify-between gap-3 rounded-xl border border-white/10 bg-black/20 px-3 py-2 text-left text-white/68 hover:bg-white/8 hover:text-white"
                            >
                              <span>邮箱</span>
                          ...
                          <div className="grid gap-2">
                            <button
                              ...
                              className="rounded-xl bg-white text-sm font-semibold text-black py-2.5 transition hover:bg-white/88"
                            >
                              修改积分
                            </button>
                            <button
                              ...
                              className="rounded-xl border border-white/12 bg-white/[0.055] py-2.5 text-sm text-white/72 transition hover:bg-white/[0.11] hover:text-white"
                            >
                              修改头像
                            </button>
                            {selectedUser.id !== currentAdminId && (
                              <button
                                ...
                                className="rounded-xl border border-purple-300/18 bg-purple-500/[0.08] py-2.5 text-sm text-purple-100 transition hover:bg-purple-500/[0.14]"
                              >
                                {selectedUser.isAdmin
                                  ? "取消管理员"
                                  : "设为管理员"}
              ...
              <div className="flex-1 min-h-0 flex flex-col gap-4">
                <section className="rounded-2xl border border-white/10 bg-white/[0.05] p-4">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-white/42">
                        输入兑换额度后生成一次性兑换码，用户在个人中心兑换后自动到账。
                      </p>
                    </div>
                  ...
                  {generatedRechargeCode && (
                    <div className="mt-4 rounded-2xl border border-emerald-400/24 bg-emerald-500/10 p-4">
                      ...
                      <div className="mt-2 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                          <p className="font-mono text-2xl font-semibold tracking-[0.12em] text-emerald-100">
                            {generatedRechargeCode.code}
                          </p>
                          <p className="mt-1 text-sm text-emerald-100/62">
                            ¥{generatedRechargeCode.amountYuan} /{" "}
                            {generatedRechargeCode.points} 积分 ·{" "}
                            {generatedRechargeCode.status === "voided"
                              ? "已作废"
                              : generatedRechargeCode.status === "redeemed"
                                ? "已兑换"
                                : "未兑换"}
                          </p>
                        </div>
                        <div className="flex flex-wrap gap-2">
                          ...
                          {generatedRechargeCode.status === "unused" && (
                            <button
                              type="button"
                              onClick={() =>
                                void voidRechargeCode(generatedRechargeCode)
                              }
                              disabled={
                                voidingRechargeCodeId ===
                                generatedRechargeCode.id
                              }
                ...
                <section className="flex-1 min-h-0 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.05]">
                  <div className="flex flex-col gap-3 border-b border-white/10 px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                      ...
                      <p className="mt-0.5 text-xs text-white/38">
                        共 {rechargeCodes.length} 条，当前显示{" "}
                        {filteredRechargeCodes.length} 条
                      </p>
                    </div>
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                      <input
                        ...
                        onChange={(event) =>
                          setRechargeCodeKeyword(event.target.value)
                        }
                      ...
                      <div className="flex rounded-xl border border-white/10 bg-black/24 p-1">
                        {[
                          {
                            key: "all" as RechargeCodeStatusFilter,
                            label: `全部 ${rechargeCodeCounts.all}`,
                          },
                          {
                            key: "unused" as RechargeCodeStatusFilter,
                            label: `未兑换 ${rechargeCodeCounts.unused}`,
                          },
                          {
                            key: "redeemed" as RechargeCodeStatusFilter,
                            label: `已兑换 ${rechargeCodeCounts.redeemed}`,
                          },
                          {
                            key: "voided" as RechargeCodeStatusFilter,
                            label: `已作废 ${rechargeCodeCounts.voided}`,
                          },
                        ].map((item) => (
                          <button
                            key={item.key}
                            type="button"
                            onClick={() =>
                              setRechargeCodeStatusFilter(item.key)
                            }
                  ...
                  {filteredRechargeCodes.length === 0 && !loading ? (
                    <div className="flex h-full items-center justify-center py-20 text-sm text-white/50">
                      暂无兑换码
                    </div>
                  ) : (
                      ...
                      <table className="w-full min-w-[980px]">
                        ...
                        <tbody>
                          {filteredRechargeCodes.map((item, idx) => (
                            <tr
                              key={item.id}
                              className={`border-b border-white/10 last:border-0 ${idx % 2 === 1 ? "bg-white/[0.06]" : ""}`}
                            >
                              <td className="px-4 py-3">
                                <button
                                  type="button"
                                  onClick={() =>
                                    void copyText(item.code, "兑换码已复制")
                                  }
                              ...
                              <td className="px-4 py-3 text-sm text-white/78">
                                ¥{item.amountYuan} / {item.points} 积分
                              </td>
                              <td className="px-4 py-3">
                                {item.status === "redeemed" ? (
                                  <span className="rounded-full border border-emerald-400/24 bg-emerald-500/12 px-2.5 py-1 text-xs text-emerald-200">
                                    已兑换
                                  </span>
                                ) : item.status === "voided" ? (
                                  <span className="rounded-full border border-red-400/24 bg-red-500/12 px-2.5 py-1 text-xs text-red-200">
                                    已作废
                                  </span>
                                ) : (
                                  <span className="rounded-full border border-blue-400/24 bg-blue-500/12 px-2.5 py-1 text-xs text-blue-200">
                                    未兑换
                                  </span>
                                )}
                              </td>
                                ...
                                <div className="flex items-center justify-end gap-2">
                                  ...
                                  {item.status === "unused" && (
                                    <button
                                      type="button"
                                      onClick={() =>
                                        void voidRechargeCode(item)
                                      }
                                      disabled={
                                        voidingRechargeCodeId === item.id
                                      }
            ...
            {activeTab === "generations" && (
              <div className="flex-1 min-h-0 flex flex-col gap-3">
                <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6 flex-shrink-0">
                  ...
                  >
                    ...
                    <div className="mt-1 text-2xl font-semibold tabular-nums text-white">
                      {globalStats.total || totalStats.total}
                    </div>
                  ...
                  >
                    ...
                    <div className="mt-1 text-2xl font-semibold tabular-nums text-sky-300">
                      {processingCount}
                    </div>
                    <div className="mt-1 text-[11px] text-white/35">
                      积压订单
                  ...
                  >
                    ...
                    <div className="mt-1 text-2xl font-semibold tabular-nums text-red-300">
                  ...
                  >
                    ...
                    <div className="mt-1 text-2xl font-semibold tabular-nums text-emerald-300">
                      {successRate}
                      <span className="ml-1 text-sm text-emerald-200/70">
                        %
                      </span>
                    </div>
                    <div className="mt-1 text-[11px] text-white/35">
                      成功 {totalStats.successCount} 单
                    </div>
                  </button>
                  ...
                  >
                    ...
                    <div className="mt-1 text-2xl font-semibold tabular-nums text-violet-200">
                      {totalStats.colorExtractionCount}
                    </div>
                    <div className="mt-1 text-[11px] text-white/35">
                      核心工具
                    </div>
                ...
                <div className="rounded-xl border border-white/10 bg-white/[0.04] p-2.5 flex-shrink-0">
                  <div className="mb-2 flex items-center justify-between gap-2 sm:hidden">
                    <div>
                      ...
                      <div className="mt-0.5 text-xs text-white/38">
                        {activeGenerationFilterCount > 0
                          ? `已启用 ${activeGenerationFilterCount} 项筛选`
                          : "默认显示全部生图记录"}
                      </div>
                    </div>
                    <button
                      type="button"
                      onClick={() =>
                        setShowGenerationFilters((current) => !current)
                      }
                  ...
                  <div
                    className={`${showGenerationFilters ? "flex" : "hidden"} flex-col gap-2 sm:flex sm:flex-row sm:flex-wrap sm:items-center`}
                  >
                  ...
                  {activeToolSummary.length > 0 && (
                    <div
                      className={`${showGenerationFilters ? "flex" : "hidden"} mt-2 flex-wrap gap-2 sm:flex`}
                    >
                  ...
                  {(totalStats.failureCount > 0 ||
                    totalStats.failureBreakdown.missingResultCount > 0) && (
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-amber-300/16 bg-amber-500/[0.075] px-3 py-2">
                        <div className="flex flex-wrap items-center gap-2 text-xs text-amber-50/78">
                          ...
                          <button
                            ...
                            className="rounded-full border border-white/10 bg-black/18 px-2 py-1 hover:bg-white/10"
                          >
                            上游{" "}
                            {totalStats.failureBreakdown.upstreamErrorCount}
                          </button>
                          <button
                            type="button"
                            onClick={() =>
                            ...
                            className="rounded-full border border-white/10 bg-black/18 px-2 py-1 hover:bg-white/10"
                          >
                            超时 {totalStats.failureBreakdown.timeoutErrorCount}
                          </button>
                          <button
                            type="button"
                            onClick={() =>
                          ...
                          >
                            无结果{" "}
                            {totalStats.failureBreakdown.missingResultCount}
                          </button>
                          <button
                            type="button"
                            onClick={() =>
                            ...
                            className="rounded-full border border-white/10 bg-black/18 px-2 py-1 hover:bg-white/10"
                          >
                            其他 {totalStats.failureBreakdown.otherFailureCount}
                          </button>
                        </div>
                        <div className="flex flex-wrap gap-2">
                          <button
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              setShowDiagnosticBreakdown((current) => !current)
                            }
                            className="rounded-full border border-white/12 bg-black/20 px-3 py-1 text-xs text-white/70 transition hover:bg-white/10 hover:text-white"
                          >
                      ...
                      {showDiagnosticBreakdown && (
                        <div className="grid gap-2 md:grid-cols-2 lg:grid-cols-4">
                          ...
                          >
                            ...
                            <div className="mt-1 text-xl font-semibold tabular-nums text-red-200">
                              {totalStats.failureBreakdown.upstreamErrorCount}
                            </div>
                          ...
                          >
                            ...
                            <div className="mt-1 text-xl font-semibold tabular-nums text-orange-200">
                              {totalStats.failureBreakdown.timeoutErrorCount}
                            </div>
                            <div className="mt-0.5 text-[11px] text-white/35">
                              超时或中断
                            </div>
                          ...
                          >
                            ...
                            <div className="mt-1 text-xl font-semibold tabular-nums text-white">
                              {totalStats.failureBreakdown.otherFailureCount}
                            </div>
                            <div className="mt-0.5 text-[11px] text-white/35">
                              人工排查
                            </div>
                  ...
                  <div className="flex-1 min-h-0 bg-white/5 rounded-xl border border-white/10 overflow-hidden relative flex flex-col">
                    {loading && records.length > 0 && (
                      <div className="absolute top-0 left-0 right-0 h-0.5 bg-white/20 z-10 overflow-hidden">
                        <div
                          className="h-full bg-gradient-to-r from-purple-500 to-blue-500 animate-[loading_1.5s_ease-in-out_infinite]"
                          style={{ width: "40%" }}
                        />
                      </div>
                    )}
                    <div className="flex-1 overflow-y-auto">
                      <div className="space-y-3 p-3 md:hidden">
                        {records.map((record) => {
                          ...
                          const resultImages = getResultImageUrls(
                            record.resultData,
                          );
                          const requestImageUrls = getRequestImageUrls(
                            record.requestParams,
                            record.uploadedImage,
                          );
                            ...
                            >
                              <div className="flex items-start justify-between gap-3">
                                ...
                                <div className="flex shrink-0 items-center gap-1.5">
                                  <span className="text-sm font-medium tabular-nums text-amber-200">
                                    {record.actualPoints > 0
                                      ? record.actualPoints
                                      : 0}
                                  </span>
                                  <SafeImage
                              ...
                              <div className="mt-3 grid grid-cols-2 gap-2">
                                ...
                                >
                                  ...
                                  {requestImageUrls.length > 1 ? (
                                    <span className="absolute bottom-2 left-2 rounded-full bg-black/60 px-2 py-0.5 text-[10px] text-white/70">
                                      {requestImageUrls.length} 张参考
                                    </span>
                                  ) : null}
                                </button>
                                ...
                                >
                                  ...
                                  {resultImages.length > 1 ? (
                                    <span className="absolute bottom-2 left-2 rounded-full bg-black/60 px-2 py-0.5 text-[10px] text-emerald-200">
                                      {resultImages.length} 张结果
                                    </span>
                                  ) : null}
                                </button>
                              </div>
                              ...
                              <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
                                <div className="min-w-0 text-xs text-white/38">
                                  {record.toolPage &&
                                  record.toolPage !== toolLabel
                                    ? `原始工具：${record.toolPage}`
                                ...
                                >
                                  ...
                                  <button
                                    type="button"
                                    onClick={() =>
                                      void downloadRecordResults(record)
                                    }
                          ...
                          <tbody>
                            {records.map((record, idx) => {
                              ...
                              const hasTimeoutError =
                                diagnostic.key === "timeout-error";
                              const hasUpstreamError =
                                diagnostic.key === "upstream-error";
                              const isMissingResult =
                                diagnostic.key === "missing-result";
                              ...
                              if (record.resultData) {
                                if (typeof record.resultData === "string") {
                                  ...
                                  record.resultData !== null
                                ) {
                                  if (
                                    Array.isArray(
                                      record.resultData.result_image_url,
                                    )
                                  ) {
                                    allImages =
                                      record.resultData.result_image_url;
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <td className="px-4 py-3 max-w-[220px]">
                                    <div className="text-sm font-mono text-white/85 truncate">
                                      {record.orderNumber}
                                    </div>
                                    <div className="mt-1 flex flex-wrap gap-1">
                                      ...
                                      {hasUpstreamError && (
                                        <span className="rounded-full border border-red-400/20 bg-red-500/10 px-2 py-0.5 text-[10px] text-red-200">
                                          上游失败
                                        </span>
                                      )}
                                      {hasTimeoutError && (
                                        <span className="rounded-full border border-orange-400/20 bg-orange-500/10 px-2 py-0.5 text-[10px] text-orange-200">
                                          超时异常
                                        </span>
                                      )}
                                      {requestImageUrls.length > 0 && (
                                        <span className="rounded-full border border-white/12 bg-white/[0.06] px-2 py-0.5 text-[10px] text-white/60">
                                          {requestImageUrls.length} 张参考
                                        </span>
                                      )}
                                    </div>
                                  ...
                                  <td className="px-4 py-3 text-right">
                                    ...
                                    {record.points !== record.actualPoints &&
                                      record.status === "成功" && (
                                  ...
                                  >
                                    <div className="flex items-center justify-center gap-1.5">
                                      ...
                                      <button
                                        ...
                                        disabled={
                                          getResultImageUrls(record.resultData)
                                            .length === 0
                                        }
                  ...
                  {editModal.type === "points" ? (
                    <div className="mb-6">
                      ...
                      <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                        ...
                        <div className="rounded-xl border border-amber-300/16 bg-amber-500/[0.08] p-3">
                          ...
                          <div className="mt-1 text-base font-semibold tabular-nums text-amber-100">
                            {editValue || 0}
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="mb-6">
                      ...
                      <input
                        ...
                        className="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-white/20"
                      />
                      {typeof editModal.currentData === "string" &&
                        editModal.currentData && (
                          <div className="mt-4">
            ...
            {detailModal.open && detailRecord && (
              ...
              >
                <div
                  className="bg-black border border-white/20 rounded-2xl p-6 max-w-4xl w-full max-h-[84vh] overflow-y-auto shadow-xl"
                  onClick={(e) => e.stopPropagation()}
                >
                  <div className="flex justify-between items-start mb-5">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
                        onClick={() =>
                          void copyText(detailCustomerSummary, "客服摘要已复制")
                        }
                  ...
                  <div className="space-y-5">
                    <div className="rounded-xl border border-cyan-300/16 bg-cyan-500/[0.075] p-4">
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        <div>
                          <div className="text-xs text-cyan-100/62">
                            客服摘要
                          </div>
                          <div className="mt-2 text-sm leading-6 text-cyan-50/86">
                            {detailDiagnostic?.label || detailRecord.status} ·{" "}
                            {detailResultImages.length} 张结果图 ·{" "}
                            {detailRequestImages.length} 张参考图 ·{" "}
                            {detailRecord.actualPoints > 0
                              ? detailRecord.actualPoints
                              : 0}{" "}
                            积分
                          </div>
                        </div>
                        <div className="flex flex-wrap gap-2">
                          <button
                            ...
                            className="rounded-lg border border-white/12 bg-black/20 px-2.5 py-1.5 text-xs text-white/72 hover:bg-white/10 hover:text-white"
                          >
                            复制订单
                          </button>
                          <button
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              void downloadRecordResults(detailRecord)
                            }
                            disabled={detailResultImages.length === 0}
                            className="rounded-lg border border-emerald-300/18 bg-emerald-500/[0.12] px-2.5 py-1.5 text-xs text-emerald-100 hover:bg-emerald-500/[0.18] disabled:cursor-not-allowed disabled:opacity-40"
                          >
                            下载结果
                          </button>
                        </div>
                      </div>
                      <pre className="mt-3 whitespace-pre-wrap rounded-lg border border-white/10 bg-black/20 p-3 text-xs leading-5 text-cyan-50/72">
                    ...
                    <div className="grid gap-3 md:grid-cols-4">
                      ...
                      <div className="rounded-xl border border-white/10 bg-white/[0.04] p-4">
                        ...
                        <div className="mt-2 text-sm">
                          ...
                          {detailRecord.points !== detailRecord.actualPoints &&
                            detailRecord.status === "成功" && (
                              <span className="ml-2 text-white/50">
                                预估 {detailRecord.points}
                              </span>
                            )}
                        </div>
                      </div>
                    ...
                    <div className="grid gap-5 lg:grid-cols-[1.1fr_1fr]">
                      <div className="space-y-4">
                        {detailDiagnostic && (
                          <div
                            className={`rounded-xl border p-4 ${detailDiagnostic.panelClass}`}
                          >
                            <div className="flex items-start justify-between gap-3">
                              <div>
                                ...
                                <div
                                  className={`mt-2 inline-flex rounded-full border px-2.5 py-1 text-xs font-medium ${detailDiagnostic.badgeClass}`}
                                >
                              ...
                              <button
                                onClick={() =>
                                  handleDiagnosticShortcut(detailDiagnostic.key)
                                }
                            ...
                            <div
                              className={`mt-3 text-sm leading-6 ${detailDiagnostic.textClass}`}
                            >
                              {detailDiagnostic.summary}
                            </div>
                            <div className="mt-3 flex flex-wrap gap-2 text-xs text-white/55">
                              <span className="rounded-full border border-white/10 bg-black/20 px-2 py-1">
                                结果图 {detailDiagnostic.resultImageCount} 张
                              </span>
                              <span className="rounded-full border border-white/10 bg-black/20 px-2 py-1">
                                参考图 {detailDiagnostic.requestImageCount} 张
                              </span>
                              <span className="rounded-full border border-white/10 bg-black/20 px-2 py-1">
                                状态 {detailRecord.status}
                              </span>
                            </div>
                            {detailDiagnostic.key !== "healthy" &&
                              detailDiagnostic.errorMessage &&
                              detailDiagnostic.errorMessage !== "未知错误" && (
                          ...
                          <div className="rounded-xl border border-sky-300/18 bg-sky-500/[0.08] p-4">
                            <div className="flex items-start justify-between gap-3">
                              <div>
                                ...
                                <div className="mt-2 flex flex-wrap gap-2 text-xs text-sky-50/72">
                                  <span className="rounded-full border border-sky-200/18 bg-black/20 px-2.5 py-1">
                                    来源{" "}
                                    {detailSmartEditAgent.promptSource ||
                                      "unknown"}
                                  </span>
                                  <span className="rounded-full border border-sky-200/18 bg-black/20 px-2.5 py-1">
                                    模式{" "}
                                    {detailSmartEditAgent.mode === "tag"
                                      ? "标记"
                                      : detailSmartEditAgent.mode === "brush"
                                        ? "画笔"
                                        : detailSmartEditAgent.mode || "-"}
                                  </span>
                                  <span className="rounded-full border border-sky-200/18 bg-black/20 px-2.5 py-1">
                                    标记 {detailSmartEditAgent.regionCount ?? 0}{" "}
                                    个
                                  </span>
                                  <span className="rounded-full border border-sky-200/18 bg-black/20 px-2.5 py-1">
                                    比例{" "}
                                    {detailSmartEditAgent.requestedAspectRatio ||
                                      "auto"}{" "}
                                    {"->"}{" "}
                                    {detailSmartEditAgent.resolvedAspectRatio ||
                                      "-"}
                                  </span>
                                  <span className="rounded-full border border-sky-200/18 bg-black/20 px-2.5 py-1">
                                    目标{" "}
                                    {detailSmartEditAgent.editTarget ||
                                      "primary"}
                                  </span>
                                </div>
                              </div>
                              <div className="text-right text-[11px] leading-5 text-sky-100/45">
                                <div>
                                  {detailSmartEditAgent.agentName ||
                                    "material-editor-prompt-agent"}
                                </div>
                                <div>
                                  {detailSmartEditAgent.agentModel || "-"} /{" "}
                                  {detailSmartEditAgent.editModel || "-"}
                                </div>
                                <div className="max-w-[280px] truncate">
                                  {detailSmartEditAgent.editBaseUrl || "-"}
                                </div>
                              </div>
                            </div>
                            ...
                            {detailSmartEditAgent.userInstruction && (
                              <div className="mt-4 rounded-lg border border-white/10 bg-black/20 p-3">
                                ...
                                <div className="mt-2 whitespace-pre-wrap break-words text-sm leading-6 text-white/82">
                                  {detailSmartEditAgent.userInstruction}
                                </div>
                              </div>
                            )}
                            ...
                            {detailSmartEditAgent.finalPrompt && (
                              <div className="mt-3 rounded-lg border border-white/10 bg-black/30 p-3">
                                ...
                                <div className="mt-2 whitespace-pre-wrap break-words text-sm leading-6 text-white/86">
                                  {detailSmartEditAgent.finalPrompt}
                                </div>
                              </div>
                            )}
                        ...
                        {detailRecord.status === "失败" && (
                          <div className="rounded-xl border border-red-500/20 bg-red-500/10 p-4">
                            ...
                            <div className="mt-2 text-sm text-red-100 break-words">
                              {getResultErrorMessage(detailRecord.resultData)}
                            </div>
                          </div>
                        )}
                      ...
                      <div className="space-y-4">
                        <div className="rounded-xl border border-white/10 bg-white/[0.04] p-4">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="text-xs text-white/35">
                              {detailResultImages.length || 0} 张
                            </div>
                          </div>
                          <div className="mt-3 grid grid-cols-2 gap-3">
                            {detailResultImages.length > 0 ? (
                              detailResultImages.map((url, index) => (
                                <button
                                  type="button"
                                  key={`${url}-${index}`}
                                  onClick={() => {
                              ...
                              <div className="col-span-2 rounded-xl border border-dashed border-white/10 px-4 py-8 text-center text-sm text-white/40">
                                暂无结果图
                              </div>
                            )}
                          </div>
                        ...
                        <div className="rounded-xl border border-white/10 bg-white/[0.04] p-4">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="text-xs text-white/35">
                              {detailRequestImages.length || 0} 张
                            </div>
                          </div>
                          <div className="mt-3 grid grid-cols-2 gap-3">
                            {detailRequestImages.length > 0 ? (
                              detailRequestImages.map((url, index) => (
                                <button
                                  type="button"
                                  key={`${url}-${index}`}
                                  onClick={() => {
                    ...
                    <details className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
                      ...
                      <div className="mt-3 overflow-auto rounded-lg border border-white/8 bg-black/30 p-3">
                        <pre className="text-xs text-white/70">
                          {JSON.stringify(detailRecord.requestParams, null, 2)}
                        </pre>
                      </div>
                    </details>
                    ...
                    <details className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
                      ...
                      <div className="mt-3 overflow-auto rounded-lg border border-white/8 bg-black/30 p-3">
                        <pre className="text-xs text-white/70">
                          {JSON.stringify(detailRecord.resultData, null, 2)}
                        </pre>
                      </div>
                    </details>
                  </div>
