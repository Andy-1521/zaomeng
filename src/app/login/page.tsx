  ...
  const handleResetPassword = async () => {
    try {
      ...
      if (result.success) {
        ...
        showToast(
          toUserFacingErrorMessage(result.message, "密码重置失败，请稍后重试"),
          "error",
        );
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-black relative overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        />
        <div
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[900px] h-[900px] bg-indigo-600/8 rounded-full blur-[120px] animate-pulse"
        ...
        <div
          className="absolute bottom-1/4 left-1/3 w-3 h-3 bg-blue-400 rounded-full animate-ping opacity-30"
          style={{ animationDelay: "0.5s" }}
        />
        <div
          className="absolute top-2/3 left-1/4 w-3 h-3 bg-pink-400 rounded-full animate-ping opacity-30"
          style={{ animationDelay: "1s" }}
        />
        <div
          className="absolute top-1/4 left-2/3 w-3 h-3 bg-indigo-400 rounded-full animate-ping opacity-30"
          style={{ animationDelay: "1.5s" }}
        />
        <div
          className="absolute bottom-1/3 right-1/4 w-3 h-3 bg-cyan-400 rounded-full animate-ping opacity-30"
          style={{ animationDelay: "2s" }}
        />

        {/* 星星闪烁效果 */}
        {starStyles.map((style, i) => (
          <div key={`star-${i}`} className="absolute" style={style}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    disabled={
                      countdown > 0 || !validateEmail(formData.email) || loading
                    }
            ...
            {mode !== "reset" && (
              <p className="text-center text-xs leading-5 text-neutral-500">
                ...
                <Link
                  href="/terms"
                  className="mx-1 text-purple-300 transition-colors hover:text-purple-200"
                >
                  用户服务协议
                </Link>
                和
                <Link
                  href="/privacy"
                  className="ml-1 text-purple-300 transition-colors hover:text-purple-200"
                >
                  隐私政策
                </Link>
              </p>
