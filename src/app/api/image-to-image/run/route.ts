...
export async function POST(request: NextRequest) {
  try {
    ...
    const orderId = body.orderId?.trim() || `AIG${Date.now()}_${Math.floor(Math.random() * 10000)}`;
    ...
    const finalPrompt = userPrompt ? `${FIXED_PROMPT}\n附加要求：${userPrompt}` : FIXED_PROMPT;
    ...
    await transactionManager.createTransaction({
      ...
      resultData: null,
      requestParams: JSON.stringify({ imageUrl, userPrompt: userPrompt || '', mode: 'image-to-image' }),
      status: '处理中',
    });

    const editedBuffer = await runPsydoImageEditFromUrl({
      imageUrl,
      prompt: finalPrompt,
      size: '1024x1792',
      quality: 'high',
    ...
    try {
      ...
      const localUrl = await saveBufferToLocalMaterialFile(editedBuffer, `grsai/${orderId}.png`);
      uploadedUrl = `http://124.223.26.206${localUrl}`;
    }
    ...
    return NextResponse.json(
      { success: false, message: error instanceof Error ? error.message : 'AI生图失败' },
      { status: 500 }
    );
  }
}
