...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const requiredPoints = getAiGeneratePoints(requestedResolution);
    ...
    const imageEditBuffer = await runPsydoImageEditFromUrl({
      ...
      quality: 'high',
      timeoutMs: IMAGE_TO_IMAGE_EDIT_TIMEOUT_MS,
      localMaterialOrigin: request.nextUrl.origin,
    });
    ...
    if (aiGeneratePointsCharged && chargedUserId && chargedPoints > 0) {
      try {
        const refundedUser = await userManager.addPointsAtomically(chargedUserId, chargedPoints);
        refundedPoints = refundedUser?.points;
        aiGeneratePointsCharged = false;
      } catch (refundError) {
        console.error('[AI生图] 失败退款异常:', refundError);
      }
    }
