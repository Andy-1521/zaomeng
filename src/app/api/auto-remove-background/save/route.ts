...
export async function POST(request: NextRequest) {
  try {
    ...
    // 生成订单号
    const orderNumber = `ARB-${Date.now()}-${Math.random().toString(36).substring(2, 8).toUpperCase()}`;
    ...
    console.error('[自动抠图保存] 保存失败:', error);
    return NextResponse.json(
      {
        success: false,
