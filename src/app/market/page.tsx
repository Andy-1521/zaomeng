...
const MARKET_HERO_FALLBACK_IMAGES: HeroMarketImage[] = [
  { id: 'fallback-phone-case-demo', title: '手机壳彩绘示例', url: '/assets/phone-case-demo.jpg' },
  { id: 'fallback-phone-case-pattern', title: '手机壳图案示例', url: '/assets/231.jpg' },
  { id: 'fallback-design-demo', title: '图案素材示例', url: '/assets/remove-watermark-demo.jpg' },
];
...
function MarketPageContent() {
  ...
  const [psdPreviewErrorByItemId, setPsdPreviewErrorByItemId] = useState<Record<string, string>>({});
  const [selectedLayerPreviewId, setSelectedLayerPreviewId] = useState('source');
  ...
  const [marketImageRatios, setMarketImageRatios] = useState<Record<string, number>>({});

  const marketThumbnailGap = marketThumbnailSize >= 300 ? 24 : marketThumbnailSize >= 240 ? 20 : 16;
  ...
  const tabs = useMemo(() => {
    ...
    if (user?.isAdmin) base.push({ key: 'pending', label: '待审核', count: pendingCount });
    return base;
  ...
  const marketMasonryColumns = useMemo(() => {
    const displayColumnCount = Math.max(1, Math.min(marketColumnCount, Math.max(items.length, 1)));
    const columns = Array.from({ length: displayColumnCount }, () => [] as MarketItem[]);
  ...
  const heroImageColumns = useMemo(() => {
    ...
    const displayImages = heroImages.length > 0 ? heroImages : MARKET_HERO_FALLBACK_IMAGES;

    return Array.from({ length: MARKET_HERO_COLUMN_COUNT }, (_, columnIndex) => {
      const columnImages = displayImages.filter((_, imageIndex) => imageIndex % MARKET_HERO_COLUMN_COUNT === columnIndex);
      const filledColumn = columnImages.length > 0 ? columnImages : [displayImages[columnIndex % displayImages.length]].filter(Boolean);

      return Array.from({ length: MARKET_HERO_ROWS_PER_COLUMN }, (_, rowIndex): HeroMarketTile => {
        const image = filledColumn[rowIndex % filledColumn.length] || displayImages[(columnIndex + rowIndex) % displayImages.length];
        if (image) {
          return {
            ...image,
  ...
  const loadMarketStats = useCallback(async () => {
    ...
    try {
      ...
      const data = await parseJsonApiResponse<ApiResponse<MarketStats>>(response, '加载图市统计失败');
  ...
  const loadItems = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/market/listings?${params.toString()}`, { credentials: 'include' });
      const data = await parseJsonApiResponse<ApiResponse<MarketItem[]>>(response, '加载图市失败');
  ...
  const scrollToBrowseMarket = useCallback(() => {
    ...
    const targetTop = browseSection.getBoundingClientRect().top + window.scrollY - 92;
  ...
  const scrollToMarketTop = useCallback(() => {
    ...
    window.setTimeout(() => {
      ...
      setShowMarketBackToTop(window.scrollY > 420 || marketBrowseLockedRef.current);
    }, 1000);
  }, []);
  ...
  const runImageSearch = useCallback(async (file: File) => {
    ...
    try {
      ...
      const data = await parseJsonApiResponse<ApiResponse<MarketItem[]>>(response, '以图搜图失败');
      if (!response.ok || !data.success || !Array.isArray(data.data)) {
        throw new Error(toUserFacingErrorMessage(data.message, '以图搜图失败'));
      }
      setItems(data.data);
      showToast(data.data.length > 0 ? '已按图案相似度排序' : '没有找到相似图案', data.data.length > 0 ? 'success' : 'info');
      setTimeout(() => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('scroll', handleMarketLockScroll, { passive: true });
  ...
  useEffect(() => {
    const updateToolbarReveal = () => {
      ...
      const progress = clampNumber((revealStart - browseTop) / Math.max(1, revealStart - revealEnd), 0, 1);
  ...
  const rememberMarketImageRatio = useCallback((itemId: string, image: HTMLImageElement) => {
    if (!image.naturalWidth || !image.naturalHeight) return;
    const nextRatio = clampNumber(image.naturalHeight / image.naturalWidth, 0.72, 1.9);
    setMarketImageRatios((current) => {
      const currentRatio = current[itemId];
      if (currentRatio && Math.abs(currentRatio - nextRatio) < 0.02) return current;
  ...
  useEffect(() => {
    localStorage.setItem('market:thumbnail-size:v2', String(marketThumbnailSize));
  }, [marketThumbnailSize]);

  useEffect(() => {
    const updateColumnCount = () => {
      const containerWidth = marketSectionRef.current?.clientWidth || window.innerWidth - 260;
      const usableWidth = Math.max(320, containerWidth - 16);
      const nextCount = clampNumber(Math.floor(usableWidth / (marketThumbnailSize + marketThumbnailGap)), 2, 10);
  ...
  const requestPsdPreview = useCallback(async (item: MarketItem, force = false) => {
    ...
    try {
      ...
      const data = await parseJsonApiResponse<ApiResponse<MarketItem>>(response, 'PSD 图层预览生成失败');
      if (!response.ok || !data.success || !data.data) {
        throw new Error(toUserFacingErrorMessage(data.message, 'PSD 图层预览生成失败'));
      }

      setItems((current) => current.map((entry) => (entry.id === item.id ? { ...entry, ...data.data } : entry)));
      setSelectedItem((current) => (current?.id === item.id ? { ...current, ...data.data } : current));
    } catch (error) {
      const message = toUserFacingErrorFromUnknown(error, 'PSD 图层预览生成失败');
      setPsdPreviewErrorByItemId((current) => ({ ...current, [item.id]: message }));
  ...
  const purchaseItem = useCallback(async (item: MarketItem) => {
    ...
    try {
      ...
      const data = await parseJsonApiResponse<ApiResponse<{ buyerRemainingPoints: number }>>(response, '购买失败');
      if (!response.ok || !data.success || !data.data) {
        throw new Error(toUserFacingErrorMessage(data.message, '购买失败'));
      }
      setPoints(data.data.buyerRemainingPoints);
      window.dispatchEvent(new CustomEvent('userPointsChanged', { detail: { points: data.data.buyerRemainingPoints } }));
      showToast('购买成功，可直接下载素材', 'success');
      await loadItems();
      setItems((current) => current.map((entry) => (entry.id === item.id ? { ...entry, purchased: true } : entry)));
      setPurchaseCompletedItemId(item.id);
      setSelectedItem((current) => current?.id === item.id ? { ...current, purchased: true } : current);
    } catch (error) {
  ...
  const downloadItem = useCallback(async (item: MarketItem, type: 'image' | 'psd') => {
    setBusyItemId(`${item.id}-${type}`);
    try {
      const params = new URLSearchParams({ itemId: item.id, type });
      const response = await fetch(`/api/market/download?${params.toString()}`, { credentials: 'include' });
      const data = await parseJsonApiResponse<ApiResponse<{ url: string }>>(response, '下载失败');
  ...
  const reviewItem = useCallback(async (item: MarketItem, status: 'approved' | 'rejected') => {
    setBusyItemId(item.id);
    try {
      const rejectionReason = status === 'rejected' ? window.prompt('请输入驳回原因', '质量或版权信息不符合上架要求') || '' : '';
      ...
      const data = await parseJsonApiResponse<ApiResponse<MarketItem>>(response, '审核失败');
      if (!response.ok || !data.success) {
  ...
  const canDownloadSelected = selectedItem && (selectedItem.purchased || selectedItem.sellerId === user?.id);
  const canPurchaseSelected = Boolean(selectedItem && selectedItem.status === 'approved' && !selectedItem.purchased && selectedItem.sellerId !== user?.id);
    ...
    <div className="relative min-h-screen overflow-hidden bg-black text-white">
      <div className="absolute inset-0">
        ...
        <div className="absolute inset-0 opacity-20" style={{
          backgroundImage: 'linear-gradient(rgba(147,51,234,.2) 1px, transparent 1px), linear-gradient(90deg, rgba(147,51,234,.2) 1px, transparent 1px)',
      ...
      <div className="relative z-10">
        ...
        <main className="pl-24 pr-8">
          <section ref={marketSectionRef} className="mx-auto max-w-[1680px] px-4 py-8">
            <style>{`
            ...
            {activeTab === 'market' && !marketBrowseLocked ? (
              <section className="relative -ml-24 -mr-8 mb-[-22vh] min-h-[calc(100vh-5rem)] overflow-hidden pb-[28vh]">
                <div className="absolute inset-0">
                  ...
                  <div className="absolute inset-y-[-24%] left-[-3%] right-[-3%] grid grid-cols-6 gap-1 opacity-70 blur-[0.2px]">
                    {heroImageColumns.map((column, columnIndex) => (
                      ...
                      >
                        {column.length > 0 ? column.map((image, imageIndex) => (
                          <div
                          ...
                          <div
                            ...
                            style={{ height: columnIndex % 2 === 0 ? 315 : 368 }}
                          />
                        ))}
                      </div>
                    ))}
                ...
                <div className="relative z-10 flex min-h-[calc(100vh-5rem)] flex-col py-5 pl-[7.25rem] pr-5 sm:pl-[8rem] sm:pr-8">
                  <div className="flex flex-1 items-center justify-center py-10">
                    <div className="w-full max-w-3xl text-center">
                      <p className="mb-4 text-sm font-medium tracking-[0.22em] text-cyan-100/72">IMAGE TO MARKET</p>
                      <h1 className="text-4xl font-semibold leading-tight tracking-normal text-white sm:text-6xl">上传主图，找到手机壳图案</h1>
                      <p className="mx-auto mt-5 max-w-2xl text-base leading-7 text-white/58 sm:text-lg">
                        自动识别电商主图里的手机壳背面彩绘，优先匹配可购买素材和 PSD。
                      </p>
                      ...
                      <div
                        ...
                        className={`mx-auto mt-8 flex min-h-[168px] max-w-2xl cursor-pointer flex-col items-center justify-center rounded-[1.7rem] border px-6 py-8 text-center backdrop-blur-2xl transition-all ${isImageSearchDragging ? 'border-cyan-200/70 bg-cyan-300/[0.16] shadow-[0_0_60px_rgba(103,232,249,0.24)]' : 'border-cyan-200/22 bg-black/42 hover:border-cyan-100/45 hover:bg-cyan-300/[0.075]'}`}
                      >
                        <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl border border-cyan-100/22 bg-cyan-300/[0.12] text-cyan-100">
                          <svg className={`h-7 w-7 ${imageSearchLoading ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                            {imageSearchLoading ? (
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.7} d="M12 3a9 9 0 1 1-6.36 2.64" />
                            ) : (
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.7} d="m21 21-4.3-4.3M10.5 18a7.5 7.5 0 1 1 0-15 7.5 7.5 0 0 1 0 15Zm-2.25-7.5h4.5M10.5 8.25v4.5" />
                            )}
                          </svg>
                        </div>
                        <p className="text-lg font-semibold text-white">{imageSearchLoading ? '正在识别主图里的图案' : '选择图片搜索'}</p>
                        <p className="mt-2 text-sm text-white/42">支持拖拽上传，适合手机壳电商主图、详情图和买家秀图。</p>
                      </div>
                      ...
                      <div className="mt-7 flex flex-wrap items-center justify-center gap-3">
                        <button
                          type="button"
                          onClick={() => imageSearchInputRef.current?.click()}
                          className="rounded-full bg-white px-5 py-2.5 text-sm font-semibold text-black transition hover:bg-cyan-50"
                        >
                          以图搜图
                        </button>
                        <button
                          ...
                          className="rounded-full border border-white/14 bg-white/[0.055] px-5 py-2.5 text-sm text-white/68 transition hover:bg-white/[0.1] hover:text-white"
                        >
                          向下逛图市
                        </button>
                      </div>
            ...
            <div
              ...
              className={`${marketToolbarPinned ? 'fixed left-4 right-4 top-[64px] md:left-24 md:right-8' : 'sticky top-20 mb-6 overflow-hidden'} z-50 flex flex-wrap items-center justify-between gap-3 rounded-[1.6rem] border border-white/10 bg-black/78 p-2 shadow-[0_18px_60px_rgba(0,0,0,0.35)] backdrop-blur-2xl transition-[opacity,transform,max-height,margin,padding] duration-200`}
              style={marketToolbarPinned ? undefined : {
                ...
                marginBottom: marketToolbarReveal > 0.12 ? '24px' : '0px',
                padding: marketToolbarReveal > 0.12 ? '8px' : '0px',
                pointerEvents: marketToolbarReveal < 0.72 ? 'none' : 'auto',
              }}
            >
              <div className="flex w-full flex-wrap items-center gap-2">
                {tabs.map((tab) => (
                  ...
                  >
                    <span>{tab.label}</span>
                    {tab.key === 'pending' && typeof tab.count === 'number' && tab.count > 0 ? (
                      <span className="rounded-full bg-amber-300 px-1.5 py-0.5 text-[10px] font-semibold leading-none text-black">{tab.count}</span>
                    ) : null}
                  </button>
                ))}
                {activeTab === 'market' ? (
              ...
              <div className="flex w-full flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex min-w-[240px] items-center gap-2 rounded-full border border-white/10 bg-white/[0.05] px-3 py-2">
                  <input
                    value={keyword}
                ...
                >
                  <svg className={`h-4 w-4 ${imageSearchLoading ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    {imageSearchLoading ? (
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.7} d="M12 3a9 9 0 1 1-6.36 2.64" />
                    ) : (
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.7} d="m21 21-4.3-4.3M10.5 18a7.5 7.5 0 1 1 0-15 7.5 7.5 0 0 1 0 15Zm-2.25-7.5h4.5M10.5 8.25v4.5" />
                ...
                <label className="hidden items-center gap-2 rounded-full border border-white/10 bg-white/[0.035] px-2.5 py-2 text-[11px] text-white/38 transition hover:bg-white/[0.055] hover:text-white/58 lg:flex" title={`缩略图大小：${marketThumbnailSize}px`}>
                  ...
                  <input
                    ...
                    onChange={(event) => setMarketThumbnailSize(Number(event.target.value))}
            ...
            {marketToolbarPinned ? <div className="h-[132px]" aria-hidden="true" /> : null}
            {imageSearchActive ? (
              <div className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-[1.3rem] border border-cyan-300/14 bg-cyan-300/[0.055] px-3 py-3">
                <div className="flex min-w-0 items-center gap-3">
                  <div className="relative h-12 w-12 shrink-0 overflow-hidden rounded-xl border border-white/10 bg-black/35">
                    {imageSearchPreviewUrl ? (
                      <img src={imageSearchPreviewUrl} alt="以图搜图参考图" className="h-full w-full object-cover" />
                    ) : null}
                  </div>
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-cyan-50">{imageSearchLoading ? '正在识别主图里的手机壳图案' : '已按手机壳图案相似度排序'}</p>
                    <p className="mt-1 truncate text-xs text-cyan-100/42">{imageSearchFileName || '电商主图'}{keyword.trim() ? ` / 关键词：${keyword.trim()}` : ''}</p>
                  </div>
                </div>
                <button
                  type="button"
            ...
            {loading ? (
              <div className="flex items-start justify-center" style={{ gap: `${marketThumbnailGap}px` }}>
                {Array.from({ length: marketColumnCount }, (_, columnIndex) => (
                  <div
                    key={`market-skeleton-column-${columnIndex}`}
                    className="min-w-0 space-y-5"
                    style={{ width: `${marketThumbnailSize}px`, maxWidth: `${marketThumbnailSize}px` }}
                  >
                    {Array.from({ length: 2 }, (_, itemIndex) => (
                      <div
                        ...
                        style={{ height: Math.max(180, Math.min(420, marketThumbnailSize * (0.86 + ((columnIndex + itemIndex) % 3) * 0.2))) }}
                      >
              ...
              <div className="rounded-[2rem] border border-white/10 bg-white/[0.035] px-8 py-16 text-center">
                ...
                <p className="mt-2 text-sm text-white/40">可以从订单结果图点击“上架”提交到图市。</p>
              </div>
            ) : (
              <div className="flex items-start justify-center" style={{ gap: `${marketThumbnailGap}px` }}>
                {marketMasonryColumns.map((column, columnIndex) => (
                  ...
                  >
	                    {column.map((item) => {
	                      const imageUrl = item.thumbnailUrl || item.previewImageUrl;
	                      const imageRatio = marketImageRatios[item.id] ?? getStableMarketImageRatio(item.id);
	                      ...
	                      return (
	                        ...
	                        >
	                          ...
	                          >
	                            <div className="relative overflow-hidden bg-black/30" style={{ aspectRatio: `${1 / imageRatio}` }}>
	                              <img
	                                src={getDisplayImageUrl(imageUrl)}
	                                alt={item.title}
	                                loading="lazy"
	                                decoding="async"
	                                onLoad={(event) => rememberMarketImageRatio(item.id, event.currentTarget)}
	                              ...
	                              {item.psdUrl ? (
	                                <span className="absolute left-3 top-3 rounded-full border border-[#31a8ff]/25 bg-[#001e36]/88 px-2 py-1 text-[11px] font-semibold text-[#31a8ff]">
	                                  PSD{item.psdLayerCount ? ` ${item.psdLayerCount}层` : ''}
	                                </span>
	                              ) : null}
	                              {item.purchased ? (
	                                <span className="absolute right-3 top-3 rounded-full border border-emerald-300/25 bg-emerald-500/22 px-2 py-1 text-[11px] text-emerald-100">已购</span>
	                              ) : null}
	                              {typeof item.similarityScore === 'number' ? (
	                                <span className="absolute bottom-3 left-3 rounded-full border border-cyan-200/24 bg-cyan-400/18 px-2 py-1 text-[11px] text-cyan-50">
	                                  相似 {Math.round(item.similarityScore * 100)}%
	                                </span>
	                              ) : null}
	                            </div>
	                          </button>
	                          <div className="space-y-2 p-3">
	                            <div className="flex items-start justify-between gap-2">
	                              <p className="line-clamp-2 text-sm font-medium text-white/86">{item.title}</p>
	                              <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-yellow-500/12 px-2 py-1 text-[11px] text-yellow-200">
	                                <Image src="/points-icon.png" alt="" aria-hidden="true" width={12} height={12} className="h-3 w-3" />
      ...
      >
        <span className="absolute left-3 top-1/2 h-px w-3 -translate-y-1/2 bg-white/34 transition-all duration-300 group-hover:w-4 group-hover:bg-[#31a8ff]/70 group-focus-visible:w-4 group-focus-visible:bg-[#31a8ff]/70" aria-hidden="true" />
        <span className="grid h-7 w-7 place-items-center rounded-full border border-white/12 bg-white/[0.06] transition-all duration-300 group-hover:ml-2 group-hover:border-[#31a8ff]/35 group-hover:bg-[#31a8ff]/12 group-focus-visible:ml-2">
          <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M12 19V5m0 0 6 6M12 5l-6 6" />
          </svg>
        </span>
      ...
      {selectedItem ? (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/76 px-4 py-6 backdrop-blur-sm" onClick={() => setSelectedItem(null)}>
          <div className="grid max-h-[92vh] w-full max-w-7xl overflow-hidden rounded-[1.55rem] border border-white/12 bg-[#07070b]/96 shadow-[0_28px_90px_rgba(0,0,0,0.55)] lg:grid-cols-[minmax(0,1fr)_360px]" onClick={(event) => event.stopPropagation()}>
            {(() => {
              const selectedLayer = selectedLayerPreviewId === 'source'
                ? null
                : selectedItem.psdLayers?.find((layer) => layer.id === selectedLayerPreviewId) || null;
              const previewUrl = selectedLayer?.previewUrl || selectedItem.previewImageUrl;
              const previewTitle = selectedLayer ? selectedLayer.name : '整图预览';
              const isLayerPreview = Boolean(selectedLayer);
              const canAccessFullPreview = selectedItem.purchased || selectedItem.sellerId === user?.id || user.isAdmin;
              ...
              return (
                <>
                  <div className="min-h-0 overflow-y-auto bg-black/28 p-4 sm:p-5">
                    <div
                      className="relative flex h-[min(64vh,720px)] min-h-[340px] items-center justify-center overflow-hidden rounded-[1.35rem] border border-white/10 bg-[#050507] shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]"
                    >
                      {isLayerPreview ? (
                        <div
                      ...
                      {previewUrl ? (
                        <SafeImage src={getDisplayImageUrl(previewUrl)} alt={previewTitle} fill sizes="min(68vw, 980px)" className="object-contain p-5" />
                      ) : (
                        <div className="flex h-full min-h-[360px] items-center justify-center text-sm text-white/34">该图层暂无预览</div>
                      )}
                      ...
                      {shouldProtectPreview ? (
                        <div className="pointer-events-none absolute inset-0 overflow-hidden">
                          <div className="absolute right-4 top-4 rounded-full border border-white/12 bg-black/58 px-3 py-1.5 text-xs text-white/62 backdrop-blur">购买前预览</div>
                          <div className="absolute inset-[-20%] grid rotate-[-18deg] grid-cols-3 gap-8 opacity-[0.11]">
                            {Array.from({ length: 18 }).map((_, index) => (
                              <span key={index} className="whitespace-nowrap text-lg font-semibold text-white">造梦AI 图市预览</span>
                            ))}
                          </div>
                        </div>
                      ) : null}
                    ...
                    <section className="mt-4 rounded-2xl border border-white/10 bg-white/[0.035] p-4 sm:p-5">
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-1 text-sm text-white/42">{selectedItem.category} / {selectedItem.sellerName || '创作者'} / {formatDate(selectedItem.createdAt)}</p>
                        </div>
                        <div className="flex flex-wrap items-center gap-2">
                          <div className="inline-flex items-center gap-2 rounded-full border border-yellow-500/20 bg-yellow-500/10 px-3 py-2 text-yellow-200">
                            <PointsIconLabel points={selectedItem.pricePoints} iconClassName="h-4 w-4" />
                          </div>
                        </div>
                      </div>

                      <p className="mt-4 whitespace-pre-wrap text-sm leading-6 text-white/58">{selectedItem.description || '购买后可用于商品铺货，不可二次转售素材文件。'}</p>

                      <div className="mt-4 flex flex-wrap gap-2">
                        {(selectedItem.tags || []).map((tag) => (
                          <span key={tag} className="rounded-full border border-white/10 bg-white/[0.055] px-2.5 py-1 text-xs text-white/48">{tag}</span>
                        ))}
                      </div>
                      ...
                      {purchaseCompletedItemId === selectedItem.id ? (
                        <div className="mt-4 rounded-2xl border border-emerald-300/20 bg-emerald-400/[0.08] p-3">
                          <p className="text-sm font-medium text-emerald-100">购买成功，素材已加入已购记录。</p>
                          <div className="mt-3 flex flex-wrap gap-2">
                            <button onClick={() => void downloadItem(selectedItem, 'image')} disabled={busyItemId === `${selectedItem.id}-image`} className="rounded-full bg-emerald-300 px-3 py-1.5 text-xs font-semibold text-black disabled:opacity-60">下载原图</button>
                            {selectedItem.psdUrl ? (
                              <button onClick={() => void downloadItem(selectedItem, 'psd')} disabled={busyItemId === `${selectedItem.id}-psd`} className="rounded-full border border-[#31a8ff]/25 bg-[#001e36] px-3 py-1.5 text-xs font-semibold text-[#31a8ff] disabled:opacity-60">下载PSD</button>
                            ) : null}
                            <button onClick={() => setActiveTab('purchased')} className="rounded-full border border-white/12 bg-white/[0.06] px-3 py-1.5 text-xs text-white/70 transition-colors hover:bg-white/[0.1]">查看已购</button>
                          </div>
                        </div>
                      ) : null}
                      ...
                      <div className="mt-5 flex flex-wrap gap-2 border-t border-white/10 pt-4">
                        {selectedItem.status === 'pending' && user.isAdmin ? (
                          <>
                            <button onClick={() => void reviewItem(selectedItem, 'approved')} disabled={busyItemId === selectedItem.id} className="rounded-full bg-emerald-500 px-4 py-2 text-sm font-semibold text-white disabled:opacity-60">通过</button>
                            <button onClick={() => void reviewItem(selectedItem, 'rejected')} disabled={busyItemId === selectedItem.id} className="rounded-full bg-red-500/80 px-4 py-2 text-sm font-semibold text-white disabled:opacity-60">驳回</button>
                          </>
                        ) : canDownloadSelected ? (
                          <>
                            <button onClick={() => void downloadItem(selectedItem, 'image')} disabled={busyItemId === `${selectedItem.id}-image`} className="rounded-full bg-white px-4 py-2 text-sm font-semibold text-black disabled:opacity-60">下载原图</button>
                            {selectedItem.psdUrl ? (
                              <button onClick={() => void downloadItem(selectedItem, 'psd')} disabled={busyItemId === `${selectedItem.id}-psd`} className="rounded-full border border-[#31a8ff]/25 bg-[#001e36] px-4 py-2 text-sm font-semibold text-[#31a8ff] disabled:opacity-60">下载PSD</button>
                            ) : null}
                          </>
                        ) : null}
                      </div>
                  ...
                  <aside className="flex min-h-0 flex-col border-t border-white/10 bg-[#0b0b12]/92 p-4 lg:border-l lg:border-t-0">
                    <div className="mb-4 flex items-start justify-between gap-3">
                      <div>
                        <p className="text-sm font-semibold text-white/88">PSD 图层</p>
                        <p className="mt-1 text-xs text-white/38">{selectedItem.psdLayerCount || selectedItem.psdLayers?.length || 0} 层 / 点击预览</p>
                      </div>
                      <button onClick={() => setSelectedItem(null)} className="rounded-full border border-white/10 bg-white/[0.06] px-3 py-1.5 text-sm text-white/55 hover:bg-white/[0.12] hover:text-white">关闭</button>
                    </div>
                    ...
                    <div className="min-h-0 flex-1 overflow-y-auto pr-1">
                      {selectedItem.psdUrl ? (
                        <div className="mb-3 grid grid-cols-2 gap-2">
                          <div className="rounded-2xl border border-white/10 bg-white/[0.035] px-3 py-2">
                            <p className="text-[11px] text-white/34">文件类型</p>
                            <p className="mt-1 text-xs font-medium text-[#9bdcff]">PSD 素材包</p>
                          </div>
                          <div className="rounded-2xl border border-white/10 bg-white/[0.035] px-3 py-2">
                            <p className="text-[11px] text-white/34">预览权限</p>
                            <p className="mt-1 text-xs font-medium text-white/70">{canAccessFullPreview ? '完整预览' : '水印预览'}</p>
                          </div>
                        </div>
                      ) : null}
                      <button
                      ...
                      >
                        <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-xl bg-black/35">
                          <SafeImage src={getDisplayImageUrl(selectedItem.thumbnailUrl || selectedItem.previewImageUrl)} alt="整图预览" fill sizes="64px" className="object-cover" />
                        </div>
                        <div className="min-w-0">
                          <p className="truncate text-sm font-medium text-white/82">整图预览</p>
                          <p className="mt-1 text-xs text-white/38">市场展示图</p>
                        </div>
                      </button>

                      {selectedItem.psdUrl ? (
                        <>
                          {selectedItem.psdLayers && selectedItem.psdLayers.length > 0 ? (
                            <div className="space-y-2">
                              {selectedItem.psdLayers.map((layer) => (
                                <button
                                  key={layer.id}
                                  type="button"
                                  onClick={() => setSelectedLayerPreviewId(layer.id)}
                                ...
                                >
                                  ...
                                  >
                                    {layer.previewUrl ? (
                                      <SafeImage src={layer.previewUrl} alt={layer.name} fill sizes="64px" className="object-contain p-1" />
                                    ) : (
                                      <div className="flex h-full items-center justify-center text-[10px] text-white/28">无预览</div>
                                    )}
                                  </div>
                                  <div className="min-w-0 flex-1">
                                    <p className="truncate text-sm font-medium text-white/78">{layer.name}</p>
                                    <div className="mt-1 flex flex-wrap items-center gap-1.5 text-xs text-white/36">
                                      <span>{layer.width}x{layer.height}</span>
                                      {layer.previewUrl ? <span className="rounded-full bg-white/[0.06] px-1.5 py-0.5 text-[10px] text-white/42">可预览</span> : null}
                                      {!layer.visible ? <span className="rounded-full bg-white/[0.06] px-1.5 py-0.5 text-[10px] text-white/42">隐藏层</span> : null}
                                    </div>
                                  </div>
                                </button>
                              ))}
                            ...
                            <div className="rounded-2xl border border-amber-300/18 bg-amber-400/[0.07] p-4">
                              <p className="text-sm text-amber-100">{psdPreviewErrorByItemId[selectedItem.id]}</p>
                              <button
                                onClick={() => void requestPsdPreview(selectedItem, true)}
                        ...
                        <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4 text-sm text-white/40">该素材未附带 PSD 文件。</div>
                      )}
                    </div>
                    ...
                    <div className="mt-4 shrink-0 rounded-2xl border border-white/10 bg-white/[0.045] p-3">
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <div className="inline-flex items-center gap-1.5 rounded-full bg-yellow-500/10 px-2.5 py-1 text-yellow-200">
                          <PointsIconLabel points={selectedItem.pricePoints} iconClassName="h-3.5 w-3.5" />
                        </div>
                      </div>
                      <div className="mt-3 flex flex-wrap gap-2">
                        {selectedItem.status === 'pending' && user.isAdmin ? (
                          <>
                            <button onClick={() => void reviewItem(selectedItem, 'approved')} disabled={busyItemId === selectedItem.id} className="flex-1 rounded-full bg-emerald-500 px-3 py-2 text-sm font-semibold text-white disabled:opacity-60">通过</button>
                            <button onClick={() => void reviewItem(selectedItem, 'rejected')} disabled={busyItemId === selectedItem.id} className="flex-1 rounded-full bg-red-500/80 px-3 py-2 text-sm font-semibold text-white disabled:opacity-60">驳回</button>
                          </>
                        ) : canDownloadSelected ? (
                          <>
                            <button onClick={() => void downloadItem(selectedItem, 'image')} disabled={busyItemId === `${selectedItem.id}-image`} className="flex-1 rounded-full bg-white px-3 py-2 text-sm font-semibold text-black disabled:opacity-60">下载原图</button>
                            {selectedItem.psdUrl ? (
                              <button onClick={() => void downloadItem(selectedItem, 'psd')} disabled={busyItemId === `${selectedItem.id}-psd`} className="flex-1 rounded-full border border-[#31a8ff]/25 bg-[#001e36] px-3 py-2 text-sm font-semibold text-[#31a8ff] disabled:opacity-60">下载PSD</button>
                            ) : null}
                          </>
                        ) : canPurchaseSelected ? (
                          <button
                          ...
                          >
                            {busyItemId === selectedItem.id ? '购买中...' : '购买素材'}
                          </button>
                        ) : (
                          <div className="w-full rounded-xl border border-white/10 bg-black/18 px-3 py-2 text-center text-xs text-white/42">当前状态暂不可购买</div>
                        )}
                      </div>
                    </div>
                  </aside>
