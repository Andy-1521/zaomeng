...
const manualInstallSteps = [
  ...
  {
    title: '打开扩展管理',
    description: 'Chrome 输入 chrome://extensions，Edge 输入 edge://extensions。',
  },
  {
    title: '加载已解压扩展',
...
const faqItems = [
  {
    question: '插件未连接怎么办？',
    answer: '确认浏览器扩展已启用，然后刷新造梦AI页面。仍未连接时，删除旧版本后重新安装最新版。',
  },
  {
    question: '保存时报未登录怎么办？',
  ...
  },
  {
    ...
    answer: '部分网页会使用背景图、懒加载或遮罩。把鼠标移到大图区域，再使用右键或悬浮采集按钮。',
  },
];

export default async function PluginPage() {
  const version = await readExtensionVersion();
  const availableStoreInstallOptions = storeInstallOptions.filter((item) => Boolean(item.href));
        ...
        <main className="mx-auto flex w-full max-w-6xl flex-col gap-5 px-4 py-6 sm:px-6 lg:px-8">
            ...
            <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_340px]">
              <div className="p-6 sm:p-8 lg:p-10">
                <div className="flex flex-wrap items-center gap-2 text-xs text-white/58">
                  ...
                  <span className="rounded-full border border-white/10 bg-white/[0.055] px-3 py-1">网页采图</span>
                  <span className="rounded-full border border-white/10 bg-white/[0.055] px-3 py-1">自动入库</span>
                </div>

                <div className="mt-7 max-w-2xl">
                  <p className="text-sm font-medium text-cyan-100/76">浏览器扩展</p>
                ...
                <div className="mt-8 max-w-2xl">
                  <p className="mb-3 text-xs font-medium tracking-[0.2em] text-emerald-100/70">从这里开始</p>
                  <a
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="block text-base font-semibold sm:text-lg">开始安装：下载插件包</span>
                      <span className="mt-1 block text-xs leading-5 text-white/50 sm:text-sm">点这里下载 zip，随后按右侧步骤解压并加载扩展。</span>
                    </span>
                ...
                {availableStoreInstallOptions.length > 0 ? (
                  <div className="mt-5 rounded-2xl border border-emerald-300/16 bg-emerald-400/[0.06] p-3">
                    <p className="text-sm font-semibold text-emerald-50">浏览器商店安装</p>
                ...
                <div className="mt-5 flex flex-wrap items-center gap-2 text-xs text-white/46">
                  ...
                  {supportedBrowsers.map((item) => (
                    <span key={item} className="rounded-full border border-white/10 bg-white/[0.045] px-3 py-1.5 text-white/64">
                      {item}
                    </span>
                  ))}
                </div>
                ...
                <div className="flex h-full flex-col gap-6">
                  <div>
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div>
                        <p className="text-sm font-semibold text-white">下载后继续</p>
                        <p className="mt-1 text-xs text-white/42">按顺序走完即可连接插件</p>
                      </div>
                    </div>

                    <div className="mt-5 space-y-4">
                      {manualInstallSteps.map((step, index) => (
                        <div key={step.title} className="grid grid-cols-[1.5rem_1fr] gap-3">
                          ...
                          <span>
                            ...
                            <span className="mt-1 block text-xs leading-5 text-white/45">{step.description}</span>
                          </span>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="rounded-2xl border border-cyan-300/16 bg-cyan-400/[0.06] px-4 py-3 text-xs leading-5 text-cyan-50/70">
                    当前版本 v{version}。安装完成后刷新页面，顶部导航栏会显示插件是否连接。
                  </div>
                </div>
              </aside>
            </div>
          ...
          <section className="grid gap-3 md:grid-cols-3">
            {quickSteps.map((step, index) => (
              <div key={step.title} className="rounded-2xl border border-white/[0.08] bg-white/[0.045] p-4 backdrop-blur-xl">
                <div className="flex items-center gap-3">
                ...
                <p className="mt-3 text-xs leading-6 text-white/52">{step.description}</p>
              </div>
            ))}
          </section>
              ...
              <div className="mt-3 divide-y divide-white/[0.08]">
                {faqItems.map((item) => (
                  <details key={item.question} className="group py-3 first:pt-0 last:pb-0">
                    ...
                    <p className="mt-2 text-xs leading-6 text-white/48">{item.answer}</p>
                  </details>
                ))}
              </div>
            </div>
