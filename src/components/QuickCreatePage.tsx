...
import {
  formatPointsLabel,
  getAiGeneratePoints,
  getColorExtractionPoints,
  getHdUpscalePoints,
  getOutpaintUpsamplingPoints,
  getRemoveBackgroundPoints,
  getSmartEditPoints,
} from "@/lib/pricing";
import {
  isSmartEditAspectRatioOption,
  isSmartEditResolution,
  type SmartEditAspectRatioOption,
  type SmartEditResolution,
} from "@/lib/smartEditSize";
...
type FailedMaterialUploadResult = Extract<
...
const galleryActions: GalleryAction[] = [
  ...
  },
  {
    ...
    preview: (
      <div className="w-32 h-full min-h-[140px] rounded-lg flex items-center justify-center overflow-hidden bg-black/20">
        <Image
          src="/assets/phone-case-demo.jpg"
          alt="彩绘提取示例"
          width={128}
          height={140}
          className="w-full h-full object-cover"
  ...
  },
  {
    ...
    className:
      "bg-gradient-to-r from-slate-700 to-zinc-600 hover:from-slate-600 hover:to-zinc-500",
    tag: "5积分",
    preview: (
...
const AI_ASPECT_RATIO_OPTIONS: Array<{
...
const AI_RESOLUTION_OPTIONS: Array<{
  value: SmartEditResolution;
  label: string;
  description: ReactNode;
}> = [
  {
...
function getActionTotalPoints(
  action: GalleryAction,
  aiResolution: SmartEditResolution,
  imageCount: number,
) {
  const points =
    action.id === "ai-generate"
      ? getAiGeneratePoints(aiResolution)
      : action.points;
...
function isAbortLikeError(error: unknown) {
  ...
  return /abort|aborted|cancel|cancelled|network|failed to fetch|load failed/i.test(
    error.message,
  );
}
...
function isDirectUploadCompletionError(
  error: unknown,
): error is DirectUploadCompletionError {
...
function readMaterialUploadSession(
  userId?: string,
): StoredMaterialUploadSession | null {
  if (typeof window === "undefined") return null;

  try {
    const raw = window.sessionStorage.getItem(
      getMaterialUploadSessionKey(userId),
    );
    if (!raw) return null;
    ...
    return {
      ...
      pendingDirectUploads: Array.isArray(parsed.pendingDirectUploads)
        ? parsed.pendingDirectUploads
        : [],
...
function writeMaterialUploadSession(session: StoredMaterialUploadSession) {
  ...
  window.sessionStorage.setItem(
    getMaterialUploadSessionKey(session.userId),
...
function removePendingMaterialUploadFromSession(
  ...
  const pendingDirectUploads = (session.pendingDirectUploads || []).filter(
    (item) => item.key !== key,
  );
...
function getMaterialFromPluginPayload(
  payload?: PluginCaptureSavedPayload | null,
): CapturedImageRecord | null {
  if (!payload) return null;

  if (
    payload.material?.id &&
    payload.material.imageUrl &&
    payload.material.createdAt
  ) {
...
async function prepareImageFileForUpload(
  ...
  try {
    ...
    const compressedFile = new File(
      [compressedBlob],
      getCompressedFileName(file.name, targetType),
      {
        type: targetType,
...
async function uploadMaterialThroughServer(
  file: File,
  options: { activeFolderId: string | null; originalFileName: string },
) {
...
async function completeDirectUploadMaterial(
  upload: {
    key: string;
    originalFileName: string;
    materialFolderId?: string | null;
  ...
  if (
    ...
    !completeData.data?.url
  ) {
    throw new Error(
      toUserFacingErrorMessage(completeData.message, "素材入库失败"),
    );
  }
...
async function completeDirectUploadMaterialWithRetry(
  ...
  for (
    let attempt = 0;
    attempt < MATERIAL_UPLOAD_COMPLETE_RETRY_DELAYS_MS.length;
    attempt += 1
  ) {
...
async function uploadMaterialDirectToOSS(
  file: File,
  options: {
  ...
  const policyData =
    (await policyResponse.json()) as DirectUploadPolicyResponse;
  if (!policyResponse.ok || !policyData.success || !policyData.data) {
    throw new Error(
      toUserFacingErrorMessage(policyData.message, "生成上传凭证失败"),
    );
  }
  ...
  formData.append(
    ...
    uploadPolicy.successActionStatus || "200",
  );
...
function QuickCreateDropdown({
  ...
  return (
    <div
      className={`relative ${isOpen ? "z-[120]" : "z-20"}`}
      data-role="quick-create-dropdown"
    >
      <button
        type="button"
        onClick={(event) => {
      ...
      >
        ...
        <span
          className={`truncate ${value ? "text-white/82" : "text-white/54"}`}
        >
          {selectedLabel}
        </span>
        ...
        <span
          className={`text-[10px] text-white/42 transition ${isOpen ? "rotate-180" : ""}`}
        >
          ▾
        </span>
      </button>
      ...
      {isOpen ? (
        <div
          className={`absolute z-[130] ${positionClassName} ${menuWidthClassName} overflow-hidden rounded-[1rem] border border-white/12 bg-[#0d0d12]/98 p-1 shadow-[0_18px_40px_rgba(0,0,0,0.4)] backdrop-blur-xl`}
        >
          {options.map((option) => {
...
function normalizeDuplicateKey(image: CapturedImageRecord): string {
  ...
  const normalizeProductPage = (value?: string | null) => {
    ...
    try {
      ...
      if (
        productId &&
        (host.includes("taobao.com") || host.includes("tmall.com"))
      ) {
  ...
  if (pageKey && hostKey && imageTypeKey)
    return `page:${hostKey}|${pageKey}|${imageTypeKey}`;
...
function getDuplicateImageGroups(images: CapturedImageRecord[]) {
  ...
  return Array.from(duplicatedTargets.values()).filter(
...
function getDisplayImageUrl(imageUrl: string): string {
  if (
    imageUrl.startsWith("/plugin-capture/") ||
    imageUrl.startsWith("/material-editor/")
  ) {
...
function getDownloadFileName(image: CapturedImageRecord): string {
  const type =
    image.imageType === "detail"
      ? "detail"
      : image.imageType === "edited"
        ? "edited"
        : "main";
  const fallbackExtension = image.imageUrl.includes(".png")
    ? "png"
    : image.imageUrl.includes(".webp") || image.imageUrl.includes(".web")
      ? "webp"
      : "jpg";
...
function getOrderDownloadFileName(
  orderNumber: string,
  toolLabel: string,
  imageUrl: string,
  index: number,
...
function extractImageUrls(value: unknown): string[] {
  ...
  if (typeof value === "object") {
    ...
    return dedupeUrls(
      candidates.flatMap((key) => extractImageUrls(record[key])),
    );
  }
...
function getOrderToolLabel(
  toolPage: string | null | undefined,
  description?: string | null,
  orderNumber?: string | null,
): string {
  ...
  if (
    toolPage === "AI生图" ||
    toolPage === "AI生图（图生图）" ||
    description?.includes("AI生图") ||
    orderNumber?.startsWith("AIG")
  ) {
    return "AI生图";
  }
  ...
  if (
    ...
    toolPage === "去除水印" ||
    toolPage === "去水印" ||
    description?.includes("去除水印") ||
    description?.includes("AI扩图") ||
    orderNumber?.startsWith("RW-")
  ) {
    return "AI扩图";
  }
  ...
  if (
    toolPage === "高清+扩图" ||
    description?.includes("高清+扩图") ||
    orderNumber?.startsWith("HDO-")
  ) {
    return "高清+扩图";
  }
  ...
  if (
    toolPage === "高清+扩图2" ||
    description?.includes("高清+扩图2") ||
    orderNumber?.startsWith("HDO2-")
  ) {
    return "高清+扩图2";
  }
  ...
  if (
    toolPage === "高清放大" ||
    description?.includes("高清放大") ||
    orderNumber?.startsWith("HD-")
  ) {
    return "高清放大";
  }
...
function getOrderStatusClass(statusLabel: string): string {
  if (statusLabel === "处理中")
    return "border-sky-300/30 bg-sky-400/18 text-sky-100";
  if (statusLabel === "失败" || statusLabel === "超时")
    return "border-red-300/30 bg-red-500/18 text-red-100";
  if (statusLabel === "部分成功")
    return "border-amber-300/30 bg-amber-400/18 text-amber-50";
...
function getMaterialUploadPlaceholderLabel(
  status: MaterialUploadPlaceholder["status"],
) {
...
function buildOrderResultCardsFromRecord(
  item: RawOrderRecord,
): OrderResultCard[] {
  ...
  const toolLabel = getOrderToolLabel(
    item.toolPage,
    item.description,
    item.orderNumber,
  );
  ...
  if (
    (statusLabel !== "成功" && statusLabel !== "部分成功") ||
  ...
  return resultImages.map((imageUrl, index) => ({
    ...
    orderId: orderNumber,
    imageUrl,
    thumbnailUrl: Array.isArray(item.thumbnailUrls)
      ? item.thumbnailUrls[index] || null
      : null,
    createdAt,
    toolLabel,
    statusLabel,
    ...
    isResultImage: true,
    downloadFileName: getOrderDownloadFileName(
      orderNumber,
      toolLabel,
      imageUrl,
      index,
...
export default function QuickCreatePage() {
  ...
  const prefetchedMaterialPaginationRef = useRef<
    Map<string, CapturedImagesPagination>
  >(new Map());
  ...
  const [materialUploadPlaceholders, setMaterialUploadPlaceholders] = useState<
    MaterialUploadPlaceholder[]
  >([]);
  ...
  const [materialsUnavailableMessage, setMaterialsUnavailableMessage] =
    useState("");
  const [materialsPagination, setMaterialsPagination] =
    useState<CapturedImagesPagination>(EMPTY_CAPTURED_IMAGES_PAGINATION);
  ...
  >({});
  const [imageSourceSizes, setImageSourceSizes] = useState<
    Record<string, ImageSourceSize>
  >({});
  const [failedImageUrls, setFailedImageUrls] = useState<Set<string>>(
    new Set(),
  );
  const [imageRetryTokens, setImageRetryTokens] = useState<
    Record<string, number>
  >({});
  const [processingAction, setProcessingAction] =
    useState<GalleryActionId | null>(null);
  ...
  const [marketListingDraft, setMarketListingDraft] =
    useState<MarketListingDraft | null>(null);
  const [isSubmittingMarketListing, setIsSubmittingMarketListing] =
    useState(false);
  const [openingMarketListingOrderId, setOpeningMarketListingOrderId] =
    useState("");
  const [deletingOrderNumber, setDeletingOrderNumber] = useState<string | null>(
    null,
  );
  const [deletingMaterialIds, setDeletingMaterialIds] = useState<Set<string>>(
    new Set(),
  );
  const [favoritingMaterialIds, setFavoritingMaterialIds] = useState<
    Set<string>
  >(new Set());
  const [downloadingImageUrls, setDownloadingImageUrls] = useState<Set<string>>(
    new Set(),
  );
  const [aiPrompt, setAiPrompt] = useState("");
  const [aiAspectRatio, setAiAspectRatio] =
    useState<SmartEditAspectRatioOption>("auto");
  ...
  const [openDropdownId, setOpenDropdownId] =
    useState<QuickCreateDropdownId | null>(null);
  ...
  const thumbnailGap =
    thumbnailSize >= 300 ? 24 : thumbnailSize >= 240 ? 20 : 16;
  ...
  const activeFolderId = materialScope.startsWith("folder:")
    ? materialScope.slice("folder:".length)
    : null;
  const activeFolder = activeFolderId
    ? materialFolders.find((folder) => folder.id === activeFolderId) || null
    : null;

  const duplicateImageCount = useMemo(() => {
    return getDuplicateImageGroups(capturedImages).reduce(
      (count, items) => count + items.length - 1,
      0,
    );
  }, [capturedImages]);
  ...
  const processingActionLabel = useMemo(
    () =>
      galleryActions.find((action) => action.id === processingAction)?.label ||
      null,
    [processingAction],
  );
  ...
  const availableOrderTools = useMemo(() => {
    return Array.from(new Set(orderResults.map((item) => item.toolLabel))).sort(
  ...
  const toolFilterOptions = useMemo<DropdownOption[]>(() => {
    return [
      ...
      ...availableOrderTools.map((tool) => ({ value: tool, label: tool })),
    ];
  }, [availableOrderTools]);
  ...
  const filteredCapturedImages = useMemo(() => {
    return capturedImages.filter((image) => {
      if (
        materialFilter !== "all" &&
        getMaterialDateGroup(image.createdAt) !== materialFilter
      )
        return false;
    ...
    return [
      {
        key: "today" as const,
        label: "今天",
        items: groupMap.get("today") || [],
      },
      {
        key: "yesterday" as const,
        label: "昨天",
        items: groupMap.get("yesterday") || [],
      },
      {
        key: "earlier" as const,
        label: "更早",
        items: groupMap.get("earlier") || [],
      },
  ...
  const hasMaterialUploadPlaceholders =
    visibleMaterialUploadPlaceholders.length > 0;
  ...
  const syncPoints = useCallback(
    (points: number) => {
      ...
      window.dispatchEvent(
        new CustomEvent("userPointsChanged", { detail: { points } }),
      );
    },
    [setPoints],
  );
  ...
  const upsertMaterialUploadPlaceholder = useCallback(
    (placeholder: MaterialUploadPlaceholder) => {
        ...
        const next = [
          ...
          ...prev.filter(
            (item) =>
              ...
              (!placeholder.key || item.key !== placeholder.key),
          ),
        ];
  ...
  const updateMaterialUploadPlaceholder = useCallback(
    (clientId: string, updates: Partial<MaterialUploadPlaceholder>) => {
      setMaterialUploadPlaceholders((prev) =>
        prev.map((item) =>
  ...
  const removeMaterialUploadPlaceholder = useCallback(
    (clientIdOrKey: string) => {
      setMaterialUploadPlaceholders((prev) =>
        prev.filter((item) => {
          const shouldRemove =
            item.id === clientIdOrKey || item.key === clientIdOrKey;
  ...
  const markMaterialUploadPlaceholderFailed = useCallback(
  ...
  const loadCapturedImages = useCallback(
    async (options?: {
      offset?: number;
      append?: boolean;
      ...
      try {
        ...
        const response = await fetch(
          `/api/plugin/captured-images?${params.toString()}`,
          { credentials: "include" },
        );
        const data = await parseJsonApiResponse<CapturedImagesResponse>(
          response,
          "素材库加载失败",
        );
        ...
        const isLocalUnavailablePreview =
          data.preview === true &&
          data.data.length === 0 &&
          Boolean(data.message);
        setMaterialsUnavailableMessage(
          isLocalUnavailablePreview
            ? data.message || "本地素材库数据源未连接"
            : "",
        );
        ...
        const nextPagination: CapturedImagesPagination = {
          ...
          nextOffset: Number(
            pagination?.nextOffset ?? offset + data.data.length,
          ),
        };

        hasLoadedMaterialsRef.current = true;
        prefetchedMaterialKeysRef.current.add(
          `${materialScope}:${materialFilter}`,
        );
        prefetchedMaterialPaginationRef.current.set(
          `${materialScope}:${materialFilter}`,
          nextPagination,
        );
        ...
        setCapturedImages((prev) => {
          ...
          if (!append) {
            ...
            return Array.from(nextById.values()).sort(
              (left, right) =>
                parseMaterialDate(right.createdAt).getTime() -
                parseMaterialDate(left.createdAt).getTime(),
            );
          }
          ...
          return [
            ...prev,
            ...nextData.filter((image) => !existingIds.has(image.id)),
          ];
        });
      } catch (error) {
        console.warn("[素材库] 加载失败:", error);
        setMaterialsUnavailableMessage(
          toUserFacingErrorFromUnknown(error, "素材库加载失败，请稍后重试"),
        );
  ...
  const loadMoreCapturedImages = useCallback(async () => {
    if (
      isLoadingMaterials ||
      isLoadingMoreMaterials ||
      !materialsPagination.hasMore
    )
      return;
    await loadCapturedImages({
      offset: materialsPagination.nextOffset,
      append: true,
    });
  }, [
    isLoadingMaterials,
    ...
    loadCapturedImages,
    materialsPagination.hasMore,
    materialsPagination.nextOffset,
  ]);

  const materialMatchesCurrentView = useCallback(
    (material: CapturedImageRecord) => {
      if (
        materialFilter !== "all" &&
        getMaterialDateGroup(material.createdAt) !== materialFilter
      ) {
        return false;
      }
  ...
  const mergeCapturedImages = useCallback(
    (materials: CapturedImageRecord[]) => {
  ...
  const prefetchMaterialScope = useCallback(
    async (scope: MaterialScope) => {
      if (!user?.id) return;

      const key = `${scope}:${materialFilter}`;
      if (
        prefetchedMaterialKeysRef.current.has(key) ||
        prefetchingMaterialKeysRef.current.has(key)
      )
        return;
  ...
  const prependUploadedMaterials = useCallback(
    (materials: CapturedImageRecord[]) => {
      const visibleMaterials = materials.filter(materialMatchesCurrentView);
      if (visibleMaterials.length === 0) return;
      const newVisibleMaterials = visibleMaterials.filter(
        (material) => !locallyInsertedMaterialIdsRef.current.has(material.id),
      );
  ...
  const loadOrderResults = useCallback(
    async (options?: { silent?: boolean; force?: boolean }) => {
      ...
      if (
        ...
        now - lastOrderResultsLoadedAtRef.current <
          ORDER_RESULTS_MIN_REFRESH_INTERVAL_MS
      ) {
        return;
      }
      ...
      const request = (async () => {
        const response = await fetch("/api/task/orders?limit=160", {
          credentials: "include",
        });
        const data = await parseJsonApiResponse<{
          ...
          message?: string;
          data?: RawOrderRecord[];
        }>(response, "刷新订单记录失败，请重试");
        if (!response.ok || !data.success || !Array.isArray(data.data)) {
          throw new Error(
            toUserFacingErrorMessage(data.message, "刷新订单记录失败，请重试"),
          );
        }
        ...
        const orderByNumber = new Map(
          data.data.map((item) => [item.orderNumber || item.id, item]),
        );
        const nextTrackedProcessingOrders = Object.fromEntries(
          Object.entries(trackedProcessingOrdersRef.current).filter(
            ([orderNumber, startedAt]) => {
              if (now - startedAt > PROCESSING_ORDER_POLL_TTL_MS) return false;
              const matchedOrder = orderByNumber.get(orderNumber);
              return (
                !matchedOrder ||
                getOrderStatusLabel(matchedOrder.status) === "处理中"
              );
            },
          ),
        );
        const hasTrackedProcessingOrders =
          Object.keys(nextTrackedProcessingOrders).length > 0;
        trackedProcessingOrdersRef.current = nextTrackedProcessingOrders;

        const hasProcessingOrdersNow = data.data.some(
          (item) => getOrderStatusLabel(item.status) === "处理中",
        );
        setHasProcessingOrders(
          hasProcessingOrdersNow || hasTrackedProcessingOrders,
        );
        const latestSettledOrder = hasProcessingOrdersNow
          ? null
          : data.data.find(
              (item) =>
                getOrderStatusLabel(item.status) !== "处理中" &&
                typeof item.remainingPoints === "number",
            );
        if (
          latestSettledOrder &&
          typeof latestSettledOrder.remainingPoints === "number"
        ) {
      ...
      try {
        ...
        if (!options?.silent) {
          showToast(
            toUserFacingErrorFromUnknown(error, "刷新订单记录失败，请重试"),
            "error",
          );
        }
      } finally {
  ...
  const createMaterialFolder = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok || !data.success)
        throw new Error(
          toUserFacingErrorMessage(data.error, "创建文件夹失败，请重试"),
        );
      setNewFolderName("");
      ...
      showToast(
        toUserFacingErrorFromUnknown(error, "创建文件夹失败，请重试"),
        "error",
      );
    }
  ...
  const renameActiveFolder = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok || !data.success)
        throw new Error(
          toUserFacingErrorMessage(data.error, "重命名失败，请重试"),
        );
      setRenamingFolderId("");
      ...
      showToast(
        toUserFacingErrorFromUnknown(error, "重命名失败，请重试"),
        "error",
      );
    }
  ...
  const deleteActiveFolder = useCallback(() => {
    if (!activeFolder) return;
    requestConfirm({
      title: `删除文件夹「${activeFolder.name}」`,
      description: "文件夹内素材会移到未分类，不会删除图片。",
      confirmLabel: "删除文件夹",
      tone: "danger",
      onConfirm: async () => {
        try {
          ...
          if (!response.ok || !data.success)
            throw new Error(
              toUserFacingErrorMessage(data.error, "删除文件夹失败，请重试"),
            );
          ...
          showToast(
            toUserFacingErrorFromUnknown(error, "删除文件夹失败，请重试"),
            "error",
          );
        }
      },
    });
  }, [activeFolder, loadMaterialFolders, requestConfirm]);
  ...
  const updateMaterials = useCallback(
    async (
      ...
      const affectedMaterials = capturedImages.filter((image) =>
        idSet.has(image.id),
      );
      ...
      try {
        ...
        if (!response.ok || !data.success)
          throw new Error(
            toUserFacingErrorMessage(data.error, "更新素材失败，请重试"),
          );
        ...
        if (typeof updates.isFavorite === "boolean") {
          prefetchedMaterialKeysRef.current.delete(
            `favorite:${materialFilter}`,
          );
          prefetchedMaterialPaginationRef.current.delete(
            `favorite:${materialFilter}`,
          );
        }
        return true;
        ...
        showToast(
          toUserFacingErrorFromUnknown(error, "更新素材失败，请重试"),
          "error",
        );
        return false;
      }
  ...
  const toggleMaterialFavorite = useCallback(
    async (image: CapturedImageRecord) => {
      ...
      setCapturedImages((prev) =>
        prev.map((item) =>
          item.id === image.id ? { ...item, isFavorite: nextFavorite } : item,
        ),
      );
      ...
      try {
        ...
        setCapturedImages((prev) =>
          prev.map((item) =>
            item.id === image.id
              ? { ...item, isFavorite: image.isFavorite }
              : item,
          ),
        );
        showToast(
  ...
  const uploadFiles = useCallback(
    async (files: File[]) => {
      ...
      if (selectedUploadFiles.length < files.length) {
        showToast(
          `已自动跳过 ${files.length - selectedUploadFiles.length} 张重复选择的图片`,
          "info",
        );
      }
      ...
      const writeCurrentUploadSession = (
        status: StoredMaterialUploadSession["status"] = "running",
      ) => {
        writeMaterialUploadSession({
          ...uploadSession,
          ...
          pendingDirectUploads: pendingDirectUploads
            .filter((item) => item.status !== "completed")
            .slice(0, 80),
          updatedAt: Date.now(),
        });
      };
      const upsertPendingDirectUpload = (
        upload: StoredDirectMaterialUpload,
      ) => {
        pendingDirectUploads = [
          upload,
      ...
      const updatePendingDirectUpload = (
        key: string,
        status: StoredDirectMaterialUpload["status"],
      ) => {
      ...
      try {
        const uploadOne = async (
          ...
          try {
            ...
            markMaterialUploadPlaceholderFailed(
              ...
              isAbortError
                ? "上传超时"
                : toUserFacingErrorFromUnknown(error, "上传失败"),
            );
            return {
              success: false,
              fileName: file.name,
              message: isAbortError
                ? "上传超时，请稍后重试"
                : toUserFacingErrorFromUnknown(error, "上传失败，请重试"),
            };
          } finally {
        ...
        const results = await runWithConcurrency(
          selectedUploadFiles,
          MATERIAL_UPLOAD_CONCURRENCY,
          uploadOne,
        );
        const successfulUploads = results.filter((result) => result.success);
        const failedUploads = results.filter(
          (result): result is FailedMaterialUploadResult =>
            !result.success && !result.interrupted,
        );
        const interruptedUploads = results.filter(
          (result): result is FailedMaterialUploadResult =>
            !result.success && Boolean(result.interrupted),
        );

        if (successfulUploads.length > 0) {
          await loadCapturedImages({ preserveCurrent: true });
          const compressedCount = successfulUploads.filter(
            (result) => result.success && result.compressed,
          ).length;
          showToast(
            `成功加入 ${successfulUploads.length} 张图片到素材库`,
            "success",
          );
          if (compressedCount > 0) {
            showToast(
              `${compressedCount} 张大图已在不改变尺寸的情况下压缩后上传`,
              "info",
            );
          }
          if (failedUploads.length > 0) {
            showToast(
              `${failedUploads.length} 张上传失败：${failedUploads[0].message}`,
              "error",
            );
          }
        ...
        const hasPendingDirectUploads = pendingDirectUploads.some(
          (item) => item.status !== "completed",
        );
        const finalStatus: StoredMaterialUploadSession["status"] =
          settledCount >= selectedUploadFiles.length && !hasPendingDirectUploads
            ? "settled"
            : "running";
    ...
    [
      ...
      loadCapturedImages,
      markMaterialUploadPlaceholderFailed,
      prependUploadedMaterials,
      removeMaterialUploadPlaceholder,
      updateMaterialUploadPlaceholder,
      upsertMaterialUploadPlaceholder,
      user?.id,
      validateImageFiles,
  ...
  const retryMaterialUploadPlaceholder = useCallback(
    async (placeholder: MaterialUploadPlaceholder) => {
      if (placeholder.key) {
        ...
        try {
          ...
          removePendingMaterialUploadFromSession(
            user?.id || readStoredUserId() || "anonymous",
            placeholder.key,
          );
  ...
  const uploadAiReferenceFiles = useCallback(
    async (files: File[]) => {
      ...
      try {
        ...
        for (const file of files) {
          ...
          if (!response.ok || !data.success || !data.data?.url) {
            throw new Error(
              toUserFacingErrorMessage(data.message, "参考图上传失败，请重试"),
            );
          }
        ...
        showToast(
          toUserFacingErrorFromUnknown(error, "参考图上传失败，请重试"),
          "error",
        );
      } finally {
  ...
  const handleAiReferenceUpload = async (
    event: React.ChangeEvent<HTMLInputElement>,
  ) => {
    const files = Array.from(event.target.files || []);
  ...
  const removeUploadedImage = async (image: CapturedImageRecord) => {
    ...
    requestConfirm({
      title: "删除这张素材？",
      description: "此操作不可恢复，图片会从当前素材库移除。",
      confirmLabel: "删除素材",
      tone: "danger",
      onConfirm: async () => {
        ...
        try {
          ...
          if (!response.ok || !data.success)
            throw new Error(
              toUserFacingErrorMessage(data.error, "删除失败，请重试"),
            );
        } catch (error) {
          setCapturedImages((prev) => {
            if (prev.some((item) => item.id === image.id)) return prev;
            return [image, ...prev].sort(
              (left, right) =>
                parseMaterialDate(right.createdAt).getTime() -
                parseMaterialDate(left.createdAt).getTime(),
            );
          ...
          showToast(
            toUserFacingErrorFromUnknown(error, "删除失败，请重试"),
            "error",
          );
        } finally {
  ...
  const downloadImageByUrl = useCallback(
    async (imageUrl: string, fileName: string) => {
      ...
      try {
        const signedResponse = await fetch(
          `/api/image/download-url?url=${encodeURIComponent(displayImageUrl)}&filename=${encodeURIComponent(fileName)}`,
          { credentials: "include" },
        );
        ...
        const remainingFeedbackMs = Math.max(
          0,
          THUMBNAIL_ACTION_FEEDBACK_MS -
            (performance.now() - feedbackStartedAt),
        );
  ...
  const downloadMaterialImage = useCallback(
    async (image: CapturedImageRecord) => {
  ...
  const downloadOrderImage = useCallback(
    async (image: OrderResultCard) => {
      await downloadImageByUrl(image.imageUrl, image.downloadFileName);
    },
    [downloadImageByUrl],
  );

  const openMarketListingDialog = useCallback(
    async (order: OrderResultCard) => {
      setOpeningMarketListingOrderId(order.orderNumber);
      let latestOrder = order;

      try {
        const response = await fetch("/api/task/orders?limit=200", {
          credentials: "include",
        });
        const data = await parseJsonApiResponse<{
          success?: boolean;
        ...
        if (response.ok && data.success && Array.isArray(data.data)) {
          const latestCards = data.data.flatMap(
            buildOrderResultCardsFromRecord,
          );
          const matchedOrder =
            latestCards.find(
              (card) =>
                card.orderNumber === order.orderNumber &&
                card.imageUrl === order.imageUrl,
            ) ||
          ...
          if (matchedOrder) {
            ...
            setOrderResults((current) =>
              current.map((card) => {
                ...
                return (
                  latestCards.find(
                    (latestCard) => latestCard.imageUrl === card.imageUrl,
                  ) || card
                );
              }),
            );
  ...
  const updateMarketListingDraft = useCallback(
    (updates: Partial<Omit<MarketListingDraft, "order">>) => {
      setMarketListingDraft((current) =>
  ...
  const submitMarketListing = useCallback(async () => {
    ...
    if (
      !Number.isFinite(marketListingDraft.pricePoints) ||
      marketListingDraft.pricePoints <= 0
    ) {
    ...
    try {
      ...
      formData.append(
        "sourceOrderNumber",
        marketListingDraft.order.orderNumber,
      );
      ...
      formData.append(
        "category",
        marketListingDraft.category.trim() || "手机壳图案",
      );
      formData.append("tags", marketListingDraft.tags.trim());
      formData.append(
        "pricePoints",
        String(Math.max(1, Math.floor(marketListingDraft.pricePoints))),
      );
      ...
      if (!response.ok || !data.success) {
        throw new Error(
          toUserFacingErrorMessage(data.message, "上架失败，请重试"),
        );
      }
      ...
      showToast(
        toUserFacingErrorFromUnknown(error, "上架失败，请重试"),
        "error",
      );
    } finally {
  ...
  const deleteOrderRecord = useCallback(
    async (image: OrderResultCard) => {
      ...
      requestConfirm({
        title: "删除这条订单记录？",
        description: "此操作不可恢复，但不会影响已经下载到本地的图片。",
        confirmLabel: "删除记录",
        tone: "danger",
        onConfirm: async () => {
          ...
          try {
            ...
            setOrderResults((prev) => {
              removedRecords = prev.filter(
                (item) => item.orderNumber === image.orderNumber,
              );
              return prev.filter(
                (item) => item.orderNumber !== image.orderNumber,
              );
            ...
            if (!response.ok || data.success === false) {
              throw new Error(
                toUserFacingErrorMessage(data.message, "删除失败，请重试"),
              );
            }
            ...
            if (removedRecords.length > 0) {
              setOrderResults((prev) => {
                ...
                const restored = [
                  ...removedRecords.filter((item) => !existingIds.has(item.id)),
                  ...prev,
                ];
    ...
    [dispatchTaskHistoryUpdated, requestConfirm],
  );
  ...
  const deleteSelectedImages = async () => {
    ...
    if (libraryView === "orders") {
      ...
      const selectedOrders = Array.from(
        new Map(
          orderResults
            .filter(
              (image) =>
                selectedSet.has(image.imageUrl) &&
                image.statusLabel !== "处理中",
            )
      ...
      requestConfirm({
        title: `删除所选 ${selectedOrders.length} 条订单记录？`,
        description: "此操作不可恢复，处理中订单已自动排除。",
        confirmLabel: "删除所选记录",
        tone: "danger",
        onConfirm: async () => {
          try {
            ...
            const deletedOrderNumbers = new Set(
              selectedOrders.map((order) => order.orderNumber),
            );
            setOrderResults((prev) =>
              prev.filter((item) => !deletedOrderNumbers.has(item.orderNumber)),
            );
            clearSelectionState();
            dispatchTaskHistoryUpdated();
    ...
    requestConfirm({
      title: `删除所选 ${selectedImageList.length} 张素材？`,
      description: "此操作不可恢复，所选素材会从当前素材库移除。",
      confirmLabel: "删除所选素材",
      tone: "danger",
      onConfirm: async () => {
        ...
        try {
          for (const imageUrl of selectedImageList) {
            const target = capturedImages.find(
          ...
          setCapturedImages((prev) =>
            prev.filter((image) => !deletedImageUrlSet.has(image.imageUrl)),
          );
  ...
  const openDuplicateReview = () => {
    ...
    const selectedIds = new Set(
      groups.flatMap((items) => items.slice(1).map((image) => image.id)),
    );
  ...
  const confirmDeleteDuplicateImages = async () => {
    ...
    const imagesToDelete = duplicateReview.groups
      .flat()
    ...
    try {
      for (const image of imagesToDelete) {
        ...
        if (!response.ok || !data.success)
          throw new Error(
            toUserFacingErrorMessage(data.error, "删除重复图片失败，请重试"),
          );
      }

      const duplicateIdSet = new Set(imagesToDelete.map((image) => image.id));
      setCapturedImages((prev) =>
        prev.filter((image) => !duplicateIdSet.has(image.id)),
      );
      ...
      showToast(
        toUserFacingErrorFromUnknown(error, "删除重复图片失败，请重试"),
        "error",
      );
    }
  };
  ...
  const previewMaterialImage = useCallback(
    (
      imageUrl: string,
      ...
      setPreviewImage({
        ...
        downloadFileName:
          downloadFileName || `zaomeng-preview.${getUrlExtension(imageUrl)}`,
      });
  ...
  const handlePluginCapture = useCallback(
    async (payload: PluginCapturePayload | null) => {
      ...
      try {
        ...
        if (
          !response.ok ||
          !data.success ||
          (!data.data?.uploadedUrl && !material?.imageUrl)
        ) {
          throw new Error(
            toUserFacingErrorMessage(
              data.error || data.message,
        ...
        showToast(
          toUserFacingErrorFromUnknown(error, "插件采图失败，请重试"),
          "error",
        );
      }
    },
      ...
      try {
        ...
        if (!response.ok) {
          if ((user.points || 0) < requiredPoints) {
            showToast(
              `积分不足，当前 ${user.points || 0}，需要 ${requiredPoints}`,
              "error",
            );
            return false;
          }
        ...
        const currentPoints = data.success
          ? data.data?.points || 0
          : user.points || 0;
        ...
        if (currentPoints < requiredPoints) {
          showToast(
            `积分不足，当前 ${currentPoints}，需要 ${requiredPoints}`,
            "error",
          );
          return false;
        }
  ...
  const ensureEnoughColorExtractionPoints = useCallback(
    (imageCount: number) => {
  ...
  const startColorExtraction = useCallback(
    async (imageUrl: string) => {
      ...
      try {
        ...
        if (!response.ok || !data.success) {
          throw new Error(
            toUserFacingErrorMessage(data.message, "彩绘提取失败，请重试"),
          );
        }
  ...
  const startOutpaintUpsampling = useCallback(
    async (imageUrl: string) => {
      if (!user?.id) return false;
      try {
        ...
        dispatchTaskHistoryUpdated();
        showToast(
          toUserFacingErrorFromUnknown(error, "暂时未能完成处理，请稍后重试"),
          "error",
        );
        return false;
      }
  ...
  const startHdUpscale = useCallback(
    async (imageUrl: string) => {
      if (!user?.id) return false;
      try {
  ...
  const startRemoveBackground = useCallback(
    async (imageUrl: string) => {
      if (!user?.id) return false;
      try {
  ...
  const startAiGenerate = useCallback(
    (
      imageUrl: string,
      prompt: string,
      options: {
  ...
  const handleRunAction = useCallback(
    async (actionId: GalleryActionId) => {
      ...
      try {
        if (actionId === "color-extraction") {
          const hasEnoughPoints = await ensureEnoughColorExtractionPoints(
          ...
          for (let index = 0; index < selectedImageList.length; index += 1) {
            ...
            const stillEnoughPoints =
              await ensureEnoughColorExtractionPoints(remainingCount);
        ...
        if (actionId === "outpaint-upsampling") {
          const hasEnoughPoints = await ensureEnoughPoints(
            getOutpaintUpsamplingPoints() * selectedImageList.length,
          );
          if (!hasEnoughPoints) return;
          let submittedCount = 0;
          for (const imageUrl of selectedImageList) {
        ...
        if (actionId === "hd-upscale") {
          const hasEnoughPoints = await ensureEnoughPoints(
            getHdUpscalePoints() * selectedImageList.length,
          );
          if (!hasEnoughPoints) return;
          let submittedCount = 0;
          for (const imageUrl of selectedImageList) {
        ...
        if (actionId === "remove-background") {
          const hasEnoughPoints = await ensureEnoughPoints(
            getRemoveBackgroundPoints() * selectedImageList.length,
          );
          if (!hasEnoughPoints) return;
          let submittedCount = 0;
          for (const imageUrl of selectedImageList) {
    ...
    [
      clearSelectionState,
      ensureEnoughColorExtractionPoints,
      ensureEnoughPoints,
      ensureUserReady,
      selectedImageList,
      startColorExtraction,
      startHdUpscale,
      startOutpaintUpsampling,
  ...
  const handleEditorComplete = useCallback(
    (resultUrl: string) => {
      const focusEditedResult = () => {
        ...
        window.setTimeout(() => {
          const target =
            imageButtonRefs.current[resultUrl] || gallerySectionRef.current;
  ...
  const handleLocalEditComplete = useCallback(
    (
      resultUrl: string,
      meta?: { orderId?: string; status?: string; remainingPoints?: number },
    ) => {
  ...
  const handleMasonryBlankClick = useCallback(
  ...
  useEffect(() => {
    const handlePluginMessage = (event: MessageEvent) => {
      ...
      if (
        ...
        !requestedLatestCaptureRef.current
      ) {
        requestedLatestCaptureRef.current = true;
        window.postMessage(
          { source: "zaomeng-web", type: "ZAOMENG_REQUEST_LATEST_CAPTURE" },
      ...
      if (
        data.type === "ZAOMENG_CAPTURE_IMAGE_SAVED" ||
        data.type === "ZAOMENG_LATEST_CAPTURE"
      ) {
      ...
      if (data.type === "ZAOMENG_CAPTURE_IMAGE_FAILED") {
        const message =
          data.payload?.error || "插件采图失败，请确认已登录后重试";
        showToast(
          toUserFacingErrorMessage(message, "插件采图失败，请重试"),
          "error",
        );
      }
    };
  ...
  useEffect(() => {
    ...
    const session =
      readMaterialUploadSession(user.id) ||
      (storedUserId && storedUserId !== user.id
        ? readMaterialUploadSession(storedUserId)
        : null) ||
      readMaterialUploadSession("anonymous");
    if (!session) return;
    const sessionUserId = session.userId || user.id;

    const recoveredMaterials = Array.isArray(session.materials)
      ? session.materials
      : [];
    let pendingDirectUploads = (
      Array.isArray(session.pendingDirectUploads)
        ? session.pendingDirectUploads
        : []
    ...
    const intervalId = window.setInterval(() => {
      ...
      void reconcilePendingDirectUploads();
      if (
        attempts >= MATERIAL_UPLOAD_RECOVERY_MAX_ATTEMPTS ||
        (pendingDirectUploads.length === 0 && session.status === "settled")
      ) {
        window.clearInterval(intervalId);
        if (pendingDirectUploads.length > 0) {
          const retryablePendingUploads = pendingDirectUploads.filter(
            (item) => item.status === "uploaded" || item.status === "saving",
          );
  ...
  useEffect(() => {
    ...
    trackedProcessingOrdersRef.current = {
      ...trackedProcessingOrdersRef.current,
      ...Object.fromEntries(
        cachedProcessingTasks.map((task) => [
          task.orderId as string,
          task.time || Date.now(),
        ]),
      ),
    };
  ...
  useEffect(() => {
    ...
    const cachedPagination =
      prefetchedMaterialPaginationRef.current.get(materialViewKey);
  ...
  useEffect(() => {
    if (!user?.id || materialFilter !== "all" || !hasLoadedMaterialsRef.current)
      return;
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("taskHistoryUpdated", handleTaskUpdate);
  ...
  useEffect(() => {
    const updateColumnCount = () => {
      const containerWidth =
        gallerySectionRef.current?.clientWidth || window.innerWidth - 260;
      const usableWidth = Math.max(320, containerWidth - 16);
      const nextCount = Math.max(
        2,
        Math.min(12, Math.floor(usableWidth / (thumbnailSize + thumbnailGap))),
      );
  ...
  useEffect(() => {
    localStorage.setItem(
      "material-library:thumbnail-size",
    ...
    const handleTaskHistoryPanelState = (event: Event) => {
      const detail =
        event instanceof CustomEvent
          ? (event.detail as { expanded?: boolean } | undefined)
          : undefined;
      setIsTaskHistoryExpanded(Boolean(detail?.expanded));
    };

    window.addEventListener(
      "taskHistoryPanelState",
      handleTaskHistoryPanelState,
    );
    return () =>
  ...
  useEffect(() => {
    const updateActionBarPosition = () => {
      ...
      const selectedRects = selectedImageList
        .map(
          (url) =>
            imageButtonRefs.current[url]?.getBoundingClientRect() || null,
        )
      ...
      const clampedCenterX = Math.min(
        Math.max(preferredCenterX, minCenterX),
      ...
      const maxTop = Math.max(
        12,
        window.innerHeight - expectedPanelHeight - 12,
      );
  ...
  return (
    <div
      className={`flex-1 px-6 py-8 overflow-y-auto ${isCompactActionBar && selectedImageList.length > 0 && !isTaskHistoryExpanded ? "pb-40" : ""}`}
    >
      ...
      {showLocalEdit && localEditImageUrl && (
        <LocalEditPanel
          imageUrl={localEditImageUrl}
          onClose={closeLocalEdit}
          onComplete={handleLocalEditComplete}
        />
      )}

      {confirmDialog.open && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/64 px-4 backdrop-blur-sm">
          <div className="w-full max-w-[420px] overflow-hidden rounded-[1.6rem] border border-white/12 bg-[#08080d]/96 shadow-[0_28px_90px_rgba(0,0,0,0.5)] ring-1 ring-white/5">
            <div className="px-5 pt-5">
              <div
                className={`mb-4 flex h-11 w-11 items-center justify-center rounded-2xl border ${
                  confirmDialog.tone === "danger"
                    ? "border-red-300/22 bg-red-500/14 text-red-100"
            ...
            <div className="mt-5 flex items-center justify-end gap-2 border-t border-white/10 bg-white/[0.025] px-5 py-4">
              <button
                type="button"
                onClick={closeConfirmDialog}
                className="rounded-xl border border-white/10 bg-white/[0.055] px-4 py-2 text-sm font-medium text-white/68 transition-colors hover:bg-white/[0.11] hover:text-white"
              >
                取消
              </button>
              <button
          ...
          <div className="flex max-h-[86vh] w-full max-w-5xl flex-col rounded-3xl border border-white/12 bg-[#09090b]/96 shadow-2xl">
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
                      {group.map((image, imageIndex) => {
                        const checked = duplicateReview.selectedIds.has(
                          image.id,
                        );
                        ...
                        return (
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              toggleDuplicateDeleteTarget(image.id)
                            }
                          ...
                          >
                            ...
                            <div
                              className={`absolute right-2 top-2 flex h-7 w-7 items-center justify-center rounded-full border transition-colors ${checked ? "border-red-300 bg-red-500 text-white" : "border-white/25 bg-black/55 text-white/45"}`}
                            >
                              {checked && (
                                <svg
                                  className="h-4 w-4"
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-white/10 px-6 py-4">
              <p className="text-sm text-white/55">
                将删除 {duplicateReview.selectedIds.size} 张图片
              </p>
              <div className="flex items-center gap-3">
                <button
                  onClick={closeDuplicateReview}
                  className="rounded-xl bg-white/10 px-4 py-2.5 text-sm text-white/72 transition-colors hover:bg-white/16 hover:text-white"
                >
                  取消
                </button>
                <button
                  ...
                  disabled={
                    ...
                    duplicateReview.selectedIds.size === 0
                  }
      ...
      >
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3 sm:mb-5">
          ...
          >
            <span className="flex h-6 w-6 items-center justify-center rounded-full bg-white/12 text-lg leading-none transition-colors group-hover:bg-white/20">
              {libraryView === "gallery" ? "+" : "↻"}
            </span>
        ...
        <div className="relative z-10 mb-4 rounded-[1.55rem] border border-white/[0.08] bg-black/24 p-2.5 shadow-[0_12px_44px_rgba(0,0,0,0.18)] backdrop-blur-2xl ring-1 ring-white/[0.03] sm:mb-5 sm:rounded-[1.8rem] sm:p-3">
          <div className="flex flex-wrap items-center justify-between gap-2.5">
            <div
              className={`scrollbar-auto-hide flex min-w-0 flex-1 items-center gap-2 pb-0.5 ${libraryView === "gallery" ? "overflow-x-auto" : "overflow-visible"}`}
            >
              {libraryView === "gallery" ? (
                <>
                  {(
                    ...
                    >
                      {label}
                      {materialScope === scope && galleryTotalCount > 0 ? (
                        <span className="ml-1 text-white/32">
                          {galleryTotalCount}
                        </span>
                      ) : null}
                    </button>
              ...
              {renamingFolderId === activeFolder.id ? (
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <button
                    ...
                    className="rounded-full bg-blue-500/80 px-3 py-1.5 text-xs text-white hover:bg-blue-400"
                  >
                    保存
                  </button>
                  <button
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={startRenameActiveFolder}
                    className="rounded-full bg-white/8 px-3 py-1.5 text-xs text-white/58 hover:bg-white/14 hover:text-white"
                  >
                    重命名
                  </button>
                  <button
                    onClick={() => void deleteActiveFolder()}
                    className="rounded-full bg-red-500/10 px-3 py-1.5 text-xs text-red-100/80 hover:bg-red-500/24 hover:text-red-50"
                  >
                    删除
                  </button>
                </div>
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3 sm:mb-6 sm:gap-4">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            <label
              className="hidden items-center gap-2 rounded-full bg-white/[0.025] px-2.5 py-1.5 text-[11px] text-white/38 transition hover:bg-white/[0.05] hover:text-white/58 lg:flex"
            ...
            {selectedImageList.length > 0 && (
              <button
                ...
                disabled={
                  processingAction !== null || deletingOrderNumber !== null
                }
                  ...
                  {Array.from({ length: 2 }, (_, itemIndex) => {
                    const height = Math.max(
                      ...
                      Math.min(
                        360,
                        thumbnailSize *
                          (0.78 + ((columnIndex + itemIndex) % 3) * 0.16),
                      ),
                    );
                    return (
                      <div
                            ...
                            >
                              ...
                              <div className="space-y-1 px-3 py-3">
                                <div className="flex items-center justify-between gap-2">
                                  <span
                                    className={`text-xs font-medium ${isFailed ? "text-red-100/80" : "text-white/72"}`}
                                  >
                                    {getMaterialUploadPlaceholderLabel(
                                      placeholder.status,
                                    )}
                                  </span>
                                ...
                                <p
                                  className="truncate text-xs text-white/38"
                                ...
                                {isFailed && (
                                  <button
                                    ...
                                    onClick={(event) => {
                                      event.stopPropagation();
                                      void retryMaterialUploadPlaceholder(
                                        placeholder,
                                      );
                                    }}
              ...
              const groupColumns = (() => {
                ...
                for (const image of group.items) {
                  ...
                  for (let i = 1; i < columns.length; i += 1) {
                    if (
                      columns[i].heightScore < columns[targetIndex].heightScore
                    ) {
                      targetIndex = i;
                    }
                  }
                ...
                <section key={group.key}>
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <span className="text-xs text-white/45 rounded-full border border-white/10 px-2.5 py-1 bg-white/[0.04]">
                      {group.items.length}{" "}
                      {libraryView === "gallery" ? "张素材" : "条记录"}
                    </span>
                  </div>

                  <div
                    className="flex items-start justify-center"
                    onClick={handleMasonryBlankClick}
                    style={{ gap: `${thumbnailGap}px` }}
                  >
                    {groupColumns.map((column, columnIndex) => (
                      ...
                      >
                        {column.map((image, index) => {
                          ...
                          const displayThumbnailUrl = getDisplayImageUrl(
                            image.thumbnailUrl || image.imageUrl,
                          );
                          const selected = selectedImages.has(image.imageUrl);
                          const imageFailed = failedImageUrls.has(
                            image.imageUrl,
                          );
                          ...
                          const canDeleteOrder =
                            isOrderCard && image.statusLabel !== "处理中";
                          const orderStatusClass = isOrderCard
                            ? getOrderStatusClass(image.statusLabel)
                            : "";
                          const imageRatio =
                            imageAspectRatios[image.imageUrl] ?? 1;
                          const isDeletingOrder =
                            isOrderCard &&
                            deletingOrderNumber === image.orderNumber;
                          const isDeletingMaterial =
                            !isOrderCard && deletingMaterialIds.has(image.id);
                          const isFavoritingMaterial =
                            !isOrderCard && favoritingMaterialIds.has(image.id);
                          const isDownloadingImage = downloadingImageUrls.has(
                            image.imageUrl,
                          );
                          const isOpeningMarketListing =
                            isOrderCard &&
                            openingMarketListingOrderId === image.orderNumber;
                          const previewDownloadFileName = isOrderCard
                            ? image.downloadFileName
                            : getDownloadFileName(image);
                          const compactCardControls =
                            thumbnailSize < 240 ||
                            thumbnailSize * imageRatio < 180;
                          const cardControlSizeClass = compactCardControls
                            ? "h-7 w-7"
                            : "h-8 w-8";
                          const cardControlIconClass = compactCardControls
                            ? "h-3.5 w-3.5"
                            : "h-4 w-4";
                          ...
                          const actionControlsVisibilityClass =
                            canHoverCardControls
                            ...
                            >
                              ...
                              {isOrderCard && (
                                <div className="absolute left-3 top-3 z-10 flex max-w-[calc(100%-11.5rem)] flex-wrap items-center gap-2">
                                  ...
                                  <span
                                    className={`rounded-full border px-2.5 py-1 text-[11px] font-medium backdrop-blur-md ${orderStatusClass}`}
                                  >
                                    {image.statusLabel}
                                  </span>
                                </div>
                              ...
                              {selected && isSelectable && (
                                <div
                                  className={`absolute z-10 flex h-7 w-7 items-center justify-center rounded-full bg-purple-500 text-white shadow-lg ring-4 ring-purple-500/18 ${isOrderCard ? "left-3 top-12" : "left-3 top-3"}`}
                                >
                                  <svg
                                    className="w-4 h-4"
                                    fill="none"
                              ...
                              <div
                                className={`absolute z-20 flex ${cardControlWrapClass} ${actionControlsVisibilityClass}`}
                              >
                                {isOrderCard && (
                                  <button
                                    type="button"
                                    ...
                                    disabled={
                                      !canDeleteOrder || isDeletingOrder
                                    }
                                  ...
                                  >
                                    {isDeletingOrder ? (
                                      <span
                                        className={`${compactCardControls ? "h-3.5 w-3.5" : "h-4 w-4"} animate-spin rounded-full border-2 border-current/25 border-t-current`}
                                      />
                                    ) : (
                                      <svg
                                        className={cardControlIconClass}
                                        fill="none"
                                        stroke="currentColor"
                                        viewBox="0 0 24 24"
                                      >
                                      ...
                                      {isFavoritingMaterial ? (
                                        ...
                                        >
                                          <path
                                            ...
                                            strokeLinejoin="round"
                                            strokeWidth={2}
                                            d="M11.48 3.499a.6.6 0 011.04 0l2.2 4.459a.6.6 0 00.452.328l4.92.715a.6.6 0 01.333 1.024l-3.56 3.47a.6.6 0 00-.173.531l.84 4.9a.6.6 0 01-.87.632l-4.4-2.313a.6.6 0 00-.558 0l-4.4 2.313a.6.6 0 01-.87-.632l.84-4.9a.6.6 0 00-.173-.53l-3.56-3.471A.6.6 0 013.9 9.001l4.92-.715a.6.6 0 00.452-.328l2.208-4.459z"
                                          />
                                        </svg>
                                      )}
                                    </button>
                                    ...
                                    {isOpeningMarketListing ? (
                                      ...
                                      >
                                        <path
                                          ...
                                          strokeLinejoin="round"
                                          strokeWidth={2}
                                          d="M4 7h16M6 7v12a1 1 0 001 1h10a1 1 0 001-1V7M9 7V5a2 2 0 012-2h2a2 2 0 012 2v2M8 12h8m-8 4h5"
                                        />
                                      </svg>
                                    )}
                                  </button>
                                  ...
                                  >
                                    ...
                                    >
                                      <path
                                        ...
                                        strokeLinejoin="round"
                                        strokeWidth={2}
                                        d="M8 3H3v5m18 0V3h-5M3 16v5h5m8 0h5v-5"
                                      />
                                      <path
                                        strokeLinecap="round"
                                        strokeLinejoin="round"
                                        strokeWidth={2}
                                        d="M9 9l-6-6m12 6l6-6M9 15l-6 6m12-6l6 6"
                                      />
                                    </svg>
                                  </button>
                                )}
          ...
          materialsPagination.hasMore && (
            <div className="mb-10 flex justify-center">
              ...
              >
                {isLoadingMoreMaterials
                  ? "加载中..."
                  : `加载更多（${galleryLoadedCount}/${galleryTotalCount}）`}
              </button>
            </div>
          )}
                  ...
                  <div className="flex shrink-0 -space-x-2">
                    {selectedPreviewImages
                      ...
                      .map((imageUrl, index) => (
                        <div
                          key={`${imageUrl}-${index}`}
                          className="relative h-8 w-8 overflow-hidden rounded-xl border border-black/55 bg-white/[0.06] shadow-[0_0_0_1px_rgba(255,255,255,0.08)]"
                        >
                          <SafeImage
        ...
        {!imageEditor.open &&
          ...
          !isTaskHistoryExpanded &&
          selectedImageList.length > 0 &&
          (actionBarPosition || isCompactActionBar) && (
            <div
              ...
              style={
                isCompactActionBar || !actionBarPosition
                  ? undefined
                  : {
                  ...
                  <div className="flex min-w-0 items-center gap-3">
                    <div className="flex shrink-0 -space-x-2">
                      {selectedPreviewImages.map((imageUrl, index) => (
                        <div
                          key={`${imageUrl}-${index}`}
                          className="relative h-9 w-9 overflow-hidden rounded-xl border border-black/55 bg-white/[0.06] shadow-[0_0_0_1px_rgba(255,255,255,0.08)]"
                        >
                          <SafeImage
                            key={`${imageUrl}-${imageRetryTokens[imageUrl] || 0}`}
                            src={getDisplayImageUrl(imageUrl)}
                    ...
                    <span className="whitespace-nowrap rounded-full border border-white/[0.08] bg-white/[0.045] px-3 py-1.5 text-xs font-medium text-white/68">
                      已选 {selectedImageList.length} 张
                    </span>
                  </div>
                    ...
                    {editorActions.map((action) => {
                      ...
                      return (
                        <button
                          ...
                          title={
                            action.points
                              ? `默认 2k 预计扣除 ${formatPointsLabel(action.points)}`
                              : undefined
                          }
                        >
                      ...
                      <div className="flex flex-wrap gap-2 flex-1 min-w-0">
                        {selectedImageList
                          ...
                          .map((imageUrl, index) => (
                            <div
                              key={`${imageUrl}-${index}`}
                              className="relative h-14 w-14 overflow-hidden rounded-xl border border-white/10 bg-black/20"
                            >
                              <SafeImage
                                key={`${imageUrl}-${imageRetryTokens[imageUrl] || 0}`}
                                src={getDisplayImageUrl(imageUrl)}
                ...
                <div className="mt-5 grid gap-3 border-t border-white/[0.06] pt-4 md:grid-cols-[auto_minmax(0,1fr)] md:items-center">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {galleryActions.map((action) => {
                      const totalPoints = getActionTotalPoints(
                        action,
                        aiResolution,
                        selectedImageList.length,
                      );
                      return (
                        <button
                          ...
                          title={
                            totalPoints
                              ? `本次预计扣除 ${formatPointsLabel(totalPoints)}`
                              : undefined
                          }
                        >
                ...
                <div className="relative flex h-[38vh] min-h-[240px] max-h-[360px] items-center justify-center overflow-hidden rounded-2xl border border-white/10 bg-black/35 p-3 lg:h-auto lg:min-h-[520px] lg:max-h-none lg:flex-1">
                  <SafeImage
                    src={getDisplayImageUrl(
                      marketListingDraft.order.thumbnailUrl ||
                        marketListingDraft.order.imageUrl,
                    )}
                    alt="上架预览"
                    width={900}
                    height={900}
                    ...
                    <section className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            updateMarketListingDraft({ psdMode: "upload" })
                          }
                      ...
                      {marketListingDraft.psdMode === "upload" ? (
                        <input
                          ...
                          onChange={(event) =>
                            updateMarketListingDraft({
                              psdFile: event.target.files?.[0] || null,
                            })
                          }
                      ...
                      {marketListingDraft.psdFile ? (
                        <p className="mt-2 text-xs text-white/42">
                          已选择：{marketListingDraft.psdFile.name}
                        </p>
                      ) : null}
                      <p className="mt-3 text-xs leading-5 text-white/35">
                        带 PSD 的素材会在详情里显示 PSD 标识，购买后可一起下载。
                      </p>
                    </section>
                  </div>
                </div>
